...
//! Boot wiring for the VHP and VHI product plugins.
//!
...
//! The VHI plugin (`use qa_vhi_product_plugin as _;`, Task 9) is wired in the
//! same way and covered the same way below.
...
// added this file's boot-test gate), restating the same values `init`'s
// `ctx.config_or_default()` would otherwise have fallen back to —
// `VhiProductPluginConfig::default()` is exactly
// `gear::DEFAULT_VENDOR`/`DEFAULT_PRIORITY`, i.e. the same values below.
const GEAR_NAME_VHI: &str = "qa-vhi-product-plugin";

const INSTANCE_ID_VHI: &str =
    "gts.cf.toolkit.plugins.plugin.v1~cf.core.qa_product.plugin.v1~cf.core._.vhi_product.v1";

const EXPECTED_VENDOR_VHI: &str = "virtuozzo-vhi";
const EXPECTED_PRIORITY_VHI: i64 = 100;
...
/// The VHI counterpart of [`exactly_one_vhp_product_plugin_gear_is_registered`]:
/// deleting `use qa_vhi_product_plugin as _;` from `registered_gears.rs` must
/// make this fail rather than silently seeing zero gears.
#[test]
fn exactly_one_vhi_product_plugin_gear_is_registered() {
    let registry = server_gear_registry();
    ...
    let matching: Vec<&str> = registry
        ...
        .iter()
        .map(GearEntry::name)
        .filter(|name| *name == GEAR_NAME_VHI)
        .collect();

    assert_eq!(
        matching.len(),
        1,
        "expected exactly one `{GEAR_NAME_VHI}` gear in the server's inventory, found {}: {:?}",
        matching.len(),
        registry
            .gears()
            .iter()
...
/// The VHI counterpart of
/// [`the_gear_registers_the_plugin_under_its_gts_instance_id`]: `init`
/// publishes one GTS instance under `INSTANCE_ID_VHI` and registers the
/// plugin object in the `ClientHub` under that same id.
#[tokio::test]
async fn the_vhi_gear_registers_the_plugin_under_its_gts_instance_id() {
    let registry = server_gear_registry();
    let gear = registry
        .get_gear(GEAR_NAME_VHI)
        .unwrap_or_else(|| panic!("`{GEAR_NAME_VHI}` must be discoverable"));
    ...
    assert_eq!(payload_gts_id(&published[0]), INSTANCE_ID_VHI);

    // Carrying the vendor and priority the dev stack's
    // `gears.qa-vhi-product-plugin` stanza sets, which are the same values
    // `VhiProductPluginConfig::default()` (`gear::DEFAULT_VENDOR`/
    // `DEFAULT_PRIORITY`) would have produced had the stanza been absent.
    assert_eq!(
        published[0].get("vendor").and_then(Value::as_str),
        Some(EXPECTED_VENDOR_VHI)
    );
    assert_eq!(
        published[0].get("priority").and_then(Value::as_i64),
        Some(EXPECTED_PRIORITY_VHI)
    );
    ...
    let resolved = hub
        .try_get_scoped::<dyn QaProductPluginV1>(&ClientScope::gts_id(INSTANCE_ID_VHI))
        .expect("the plugin must resolve under its GTS instance id");

    // …and it is the VHI plugin, not merely *something*. These six keys are
    // `qa-vhi-product-plugin/src/schemas.rs`'s `credential_schema()`, in
    // declaration order.
    let keys: Vec<String> = resolved
        .credential_schema()
        .into_iter()
    ...
    assert_eq!(
        ...
        vec![
            ...
            "vinfra_username".to_owned(),
            "vinfra_password".to_owned(),
        ]
    );

    // The scope is a key, not a wildcard: nothing resolves under a
    // neighbouring id, and the VHI instance does not answer for the VHP one.
    assert!(
        hub.try_get_scoped::<dyn QaProductPluginV1>(&ClientScope::gts_id(&format!(
            "{INSTANCE_ID_VHI}x"
        )))
    ...
    assert!(
        hub.try_get_scoped::<dyn QaProductPluginV1>(&ClientScope::gts_id(INSTANCE_ID))
            .is_none(),
        "the VHP instance id must not resolve against a hub the VHI gear alone populated"
    );
}
