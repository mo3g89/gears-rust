...
- **Requirements Covered**:
  ...
  - [ ] `p1` - `cpt-cf-qa-nfr-dispatch-latency` — **narrowed, not closed; still deliberately unticked.** The 5 s cadence is shipped and its knob has a floor. What the observability phase added is a **queue-residency** series (`qa_runs_queue_wait_duration_seconds`, declared in qa-runs' `domain::metrics`) measuring the queue row's `enqueued_at` to the instant the drain recorded the execution as started. **It is not this requirement's quantity, and the direction of the error is known on each path.** The NFR is stated over *platform release → execution request*; the series shares neither endpoint. For a run enqueued while its platform was **occupied** — the common case — the residency contains the whole NFR window plus the predecessor's remaining runtime, so the series **over-states** the requirement and an alert on it cannot miss a violation. For a run enqueued while its platform was already **free** the residency starts *after* the release the NFR measures from, so the series **under-states** the requirement and an alert on it **can** miss a violation — and that case is ordinary, not exotic, because `queue::decide_admission` queues any launch once `queued_depth > 0`, before occupancy is consulted at all (strict FIFO, so a later arrival cannot overtake). The three ways it arises: a tick stopped at the concurrency cap, a tick whose executor listing failed, and simply the gap between sweeps. Tightening it needs a lease-release instant from qa-environments — this gear reads the lease as free-or-held and never *since when* — which is a cross-gear change the phase put out of scope. **So there is still no benchmark, load test or observation of this NFR's own quantity anywhere in the gear**, and the requirement is still met by a design argument. What has changed is that the argument now has a bounded, two-sided proxy beside it, documented at the constant and in DESIGN §3.9.
