...
- **Purpose**: Stand up qa-environments: target platform registry with credstore-referenced kubeconfigs, platform/pipeline variables, lease state, version polling. Foundation for every launch decision.
...
- **Domain Model Entities**: TargetPlatform, PlatformVariable, PipelineVariable, platform lease

- **Design Components**:

  - [x] `p1` - `cpt-cf-qa-component-environments`

- **API**: `/qa/v1/platforms*` (CRUD, variables, lease view)

- **Data**: `platforms`, `platform_variables`, `pipeline_variables`, `platform_leases`
...
- `runs-core` requires `environments` and `catalog`: launch resolution needs platforms/leases/variables and plans/TEST_META/bundles.
