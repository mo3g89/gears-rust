...
| `cpt-cf-qa-fr-env-platforms` | Environment metadata in qa-environments DB; kubeconfig in credstore by reference; lease state owned by qa-environments, mutated via SDK by the qa-runs dispatcher |
...
No QA Platform gear may depend on `kube`/`k8s-openapi` or assume a Kubernetes environment. Target-environment kubeconfigs are opaque secrets passed by reference to the execution plane.
...
| Environment | qa-environments | Metadata + credstore kubeconfig reference + lease state |
| EnvironmentVariable / PipelineVariable | qa-environments | Env-assembly inputs |
...
- Run → Environment (by ID): where; QueueEntry → (Run, Environment): waiting position
...
##### Responsibility boundaries
Does not parse repository content (catalog), does not own environment metadata or lease storage (environments — mutated via its SDK), does not build analytical aggregates (insights), does not talk to JIRA/SMTP.
...
##### Why this component exists
Target environments are long-lived shared assets with secret credentials and occupancy state; several consumers (runs dispatcher, UI, version poller) need one authority for them.

##### Responsibility scope
Environment CRUD (metadata in DB, kubeconfig in credstore by reference), environment/pipeline variables, lease state (free / held / held-exclusively) with acquire/release operations exposed via SDK, environment version polling (leader-elected background task) with change events.
...
| CRUD | `/qa/v1/environments*` | environments | Target environments + variables + lease view | unstable |
| GET | `/qa/v1/dashboard` | insights | Dashboard aggregate (recent runs, counts, pass rates, active/queued) | unstable |
| GET | `/qa/v1/dashboard/coverage` | insights | Coverage view (tests/plans × versions × environments) | unstable |
...
#### Target environments (systems under test)
...
**snapshotted from the target environment at launch** — added 2026-08-13 (qa-runs
...
`platform_id` on read would let an environment upgrade silently change a queued
...
`qa_environments_sdk::TargetPlatform` (now `Environment`) exposed `observed_version` but no build
...
**qa-environments**: `environments` (name, product_id?, description?, kubeconfig_credstore_ref, availability, observed_version, observed_build, default_branch?), `environment_variables` (platform_id, name, value — unique on `(tenant_id, platform_id, name)`), `pipeline_variables` (name, value), `environment_leases` (platform_id, mode parallel|exclusive, holder run_ids JSONB).

Four amendments to that line. `observed_build` was added by Task 9b on
2026-08-13 (see the qa-runs paragraph above). `environment_variables`' unique index
...
§3.4 rule 1's middle tier (explicit → **environment default** → repository
...
`NewEnvironment`, `EnvironmentPatch` and the REST request type.
...
`environment_leases` is keyed on a bare `platform_id`, i.e. the lease is **not
tenant-partitioned** — deliberate, since a physical environment is a single
...
Environment ownership is therefore validated in qa-runs' launch path, not by any
...
a target environment belongs to exactly one tenant.** Every property above is benign
under that invariant and only under it. It is currently implied by
qa-environments' PEP and asserted nowhere. `environment_leases` is deliberately keyed
on a bare `platform_id` and is *not* tenant-partitioned, so if the product ever
wants genuinely shared environments, `queue_max_depth`, FIFO and the lease must be
...
|--------|------|--------|------------------|
| `qa_runs_dispatch_total` | counter | `outcome` = started / refused / failed | The dispatcher's cycle rate and how each cycle ended. One increment is one **tick**, not one submission — `dispatch_one` runs inside the drain and is not counted. `refused` is the concurrency cap doing its job; `failed` is this gear's own fault, split from `refused` by `DomainError::disclosable` |
...
| `qa_runs_queue_wait_total` | counter | none | Queued runs leaving the queue **successfully**, one per drained row. Departures, not arrivals: a claimed row whose dispatch failed and a queued row the TTL sweep expired are both absent, so it equals the enqueue rate only in steady state |
...
| `qa_environments_plugin_call_duration_seconds` | histogram | `class`, as above | The plugin round trip alone. Subtract it from the per-environment observation above to get this gear's own overhead |
...
**Neither the plugin families nor the environment ones carry a plugin identity**, and that is measured rather than assumed: every product plugin in this platform composes its instance id from the *same* `QaProductPluginSpecV1` type id, so a `type` label would be a dimension with one value, and the part that distinguishes two plugins is the instance tail — an operator-supplied `VARCHAR(512)` read out of a database column, which is bounded by nothing. Revisit when a second product-plugin spec type exists.
