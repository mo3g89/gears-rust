...
# the same Argo cluster so decision D4's runner-kubeconfig `Secret` can be
...
# qa-environments to have written the kubeconfig Secret that Workflow's pod
...
# (qa-environments/src/infra/observer/secret_writer.rs:94, :111) treats an
...
if [[ -n "$QA_ENVIRONMENTS_ARGO_CONFIG" ]]; then
    ...
    if [[ -f "$IN_CLUSTER_TOKEN_FILE" ]]; then
        ...
        validate_fragment QA_ENVIRONMENTS_ARGO_CONFIG "$QA_ENVIRONMENTS_ARGO_CONFIG" 'gears.qa-environments.config' \
            ...
            "A fragment without a kubeconfig_path leaves qa-environments' D4 Secret writer on Config::infer(), which finds no service-account token ('$IN_CLUSTER_TOKEN_FILE' does not exist), no \$KUBECONFIG and no ~/.kube/config inside this container -- so every runner-kubeconfig Secret write fails, on every cycle, while observation keeps working and the stack looks healthy"
    fi
