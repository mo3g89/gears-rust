...
# `secret_writer::ensure_kubeconfig_secret` in
# qa-environments/src/infra/observer/secret_writer.rs --
...
if ! grep_count deploy/qa-platform-gears 'failed to apply this environment.s runner kubeconfig Secret' /usr/local/bin/cf-gears-example-server "gears binary: 'failed to apply...' D4 string count"; then
    exit 1
fi
...
if [ "$n_d4_platforms" -eq 0 ]; then
    ...
    secret_line="$(grep 'failed to apply this environment.s runner kubeconfig Secret' "$WORKDIR/gears-d4.log" | tail -n1 || true)"
    if [ -n "$infer_line" ]; then
        ...
        echo "PASS: with $n_d4_platforms environment(s) registered, the gears' log (read successfully, $(wc -l < "$WORKDIR/gears-d4.log" | tr -d ' ') lines) carries neither the Config::infer() failure nor a failed runner-kubeconfig Secret write -- qa-environments' D4 writer is reaching $ARGO_NAMESPACE and succeeding"
    fi
fi
...
for verb in create patch; do
    ...
    if [ "$rc" -ne 0 ] || ! grep -qx 'yes' "$WORKDIR/cani-secrets-$verb.txt"; then
        echo "FAIL: qa-platform-gears CANNOT '$verb' secrets in namespace $ARGO_NAMESPACE: $(cat "$WORKDIR/cani-secrets-$verb.txt" 2>/dev/null). qa-environments writes each environment's runner kubeconfig there (decision D4, secret_writer::ensure_kubeconfig_secret); without this grant every workflow run hangs on FailedMount while the rest of the stack looks healthy. rbac-argo.yaml's Role is what grants it." >&2
        exit 1
    fi
done
