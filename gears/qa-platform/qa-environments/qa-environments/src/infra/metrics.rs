...
/// The `class` label key, carried by the per-environment duration histogram and
/// by both plugin-call families.
///
...
///
/// The plugin-call families reuse it rather than inventing a third word because
/// their label answers the same *kind* of question — what happened, at the
/// plugin contract's own granularity — over a narrower population. The two
/// families are told apart by their names, which is where the distinction
/// belongs; a second word for one concept would make a dashboard's `class`
/// filter silently miss half the data.
...
///
/// **The plugin-call duration shares the set rather than getting one of its
/// own**, and that is deliberate rather than economical. Its samples are a
/// strict subset of the per-environment observation's — the round trip inside
/// the observation that contains it — so its range is contained in that
/// family's range by construction, and sharing the boundaries is what makes the
/// two comparable bucket for bucket. Subtracting one from the other is the
/// operation both constants' docs are written around, and it is only honest
/// arithmetic if the two are bucketed alike.
///
...
pub struct QaEnvironmentsMetricsMeter {
    ...
    plugin_call: Counter<u64>,
    plugin_call_duration: Histogram<f64>,
}
    ...
    pub fn new(meter: &Meter) -> Self {
        Self {
            ...
            observation_duration: meter
                ...
                .build(),
            plugin_call: meter
                .u64_counter(QA_ENVIRONMENTS_PLUGIN_CALL)
                .with_description(
                    "Calls into a product plugin's observe, by what the plugin answered; \
                     the failure classes are the plugin contract's own",
                )
                .build(),
            plugin_call_duration: meter
                .f64_histogram(QA_ENVIRONMENTS_PLUGIN_CALL_DURATION)
                .with_description(
                    "Wall-clock duration of one product-plugin observe round trip, in \
                     seconds, and of nothing around it",
                )
                .with_boundaries(DURATION_BUCKETS.to_vec())
                .build(),
        }
...
impl PluginMetrics for QaEnvironmentsMetricsMeter {
    fn plugin_call(&self, class: PluginCallClass, duration: Duration) {
        let labels = [KeyValue::new(CLASS, class.as_str())];
        self.plugin_call.add(1, &labels);
        self.plugin_call_duration
