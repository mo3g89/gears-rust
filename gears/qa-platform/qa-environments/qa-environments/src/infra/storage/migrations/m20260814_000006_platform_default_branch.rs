...
//! (`manager/src/services/platforms.rs:845-848`) — but this gear shipped
//! without it. Left alone, a launch naming no branch silently used the
//! *repository's* default where the source system would have used the
//! platform's: no error, no warning, and visible only to an operator who had
//! pinned a platform to a release branch and then wondered why `main` ran.
//!
...
//! **operator-set** and therefore threads through `NewPlatform`,
//! `PlatformPatch` and the REST request type. Copying `observed_build`'s
...
//! it, `qa-runs/src/domain/service/launch.rs:1693` assigns it as `test_version`,
...
//! `PlatformsService::validate_default_branch` enforces the same 512 in
...
/// column names are runtime strings, so `platform::Model::default_branch`
...
mod tests {
        ...
        for (dialect, raw) in [
            ...
            assert!(
                ...
                "{dialect} must leave default_branch nullable -- 'no override' is \
                 the normal state, and NULL is how PlatformPatch's clear is stored"
            );
            assert!(
                sql.contains("NULL"),
    ...
    fn platform_am(default_branch: Option<String>) -> platform::ActiveModel {
        platform::ActiveModel {
            ...
            default_branch: ActiveValue::Set(default_branch),
    ...
    async fn default_branch_round_trips_through_the_migrated_schema() {
        ...
        platform_am(Some("release-9.0".to_owned()))
            .insert(&conn)
            .await
            .unwrap();
        ...
        assert_eq!(
            stored.default_branch.as_deref(),
            Some("release-9.0"),
