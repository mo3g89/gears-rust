...
impl LeasesRepository for OrmLeasesRepository {
    async fn get<C: DBRunner>(
        ...
        let found = LeaseEntity::find()
            .filter(
                sea_orm::Condition::all().add(Expr::col(LeaseColumn::PlatformId).eq(platform_id)),
            )
            .secure()
            .scope_with(scope)
            .one(runner)
        ...
        match found {
            ...
            // Missing row: no lease has ever been written for this platform.
    ...
    async fn compare_and_set<C: DBRunner>(
        ...
        if expected_version == 0 {
            ...
            let am = platform_lease::ActiveModel {
        ...
        let result = LeaseEntity::update_many()
            .filter(
                sea_orm::Condition::all()
                    .add(Expr::col(LeaseColumn::PlatformId).eq(platform_id))
