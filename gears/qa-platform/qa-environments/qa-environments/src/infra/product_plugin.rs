//! The `ProductPluginPort` adapter: `qa-catalog`'s resolver, reached through
//! the `ClientHub`.
//!
...
//! `kube` or `k8s-openapi` — `cargo tree -p qa-environments -i kube` still
//! reports no such package in a default build — so the containment argument is
    ...
    async fn plugin_for(
        ...
        let resolver = match self.client_hub.get::<dyn QaProductPluginResolverV1>() {
            ...
            Err(error) => {
                warn!(
                    %error,
                    "qa-environments: no product-plugin resolver is registered in the \
                     ClientHub; this deployment has no qa-catalog beside this gear"
                );
        ...
        resolver.plugin_for(ctx, product_id).await.map_err(|error| {
            warn!(
                %product_id,
                %error,
                "qa-environments: this environment's product plugin could not be resolved"
            );
