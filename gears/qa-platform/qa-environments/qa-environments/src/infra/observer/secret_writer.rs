...
//!
//! **This targets the Argo cluster, not the platform's.** [`KubeObserver::observe`]
//! (`kube_observer.rs`) builds a client from a *platform's* kubeconfig — the
//! cluster under test. This module builds a *different* client, from
//! `argo_kubeconfig_path`, aimed at the cluster where `qa-runs`' test-runner
//! pods actually execute. Conflating the two would write the Secret into the
//! wrong cluster and the symptom would be identical to today's bug: a pod
//! stuck `Pending` on `FailedMount`, just for a different reason.
//!
...
fn secret_name(prefix: &str, reference: &str) -> String {
    truncate(&sanitize(&format!("{prefix}{reference}")), MAX_SECRET_NAME_LEN)
}
...
/// on every self-heal cycle. Same defect as the one measured on the platform
...
async fn argo_client(argo_kubeconfig_path: Option<&str>) -> Result<Client, String> {
    let trimmed = argo_kubeconfig_path.map(str::trim).filter(|path| !path.is_empty());
    let config = match trimmed {
        ...
        None => Config::infer()
            .await
            .map_err(|_| format!("failed to infer a Kubernetes config for the Argo cluster: {INFER_FAILURE}"))?,
    };
...
pub(super) async fn ensure_kubeconfig_secret(
    ...
    data.insert(secret_key.to_owned(), ByteString(kubeconfig.as_bytes().to_vec()));
...
mod tests {
    ...
    fn conflict_error() -> kube::Error {
        kube::Error::Api(
            kube::core::Status::failure(
                "secrets \"qa-platform-x\" already exists",
                "Conflict",
            )
            .with_code(409)
            .boxed(),
    ...
    async fn a_missing_argo_kubeconfig_file_is_a_named_failure_not_a_panic() {
        let result = ensure_kubeconfig_secret(
            ...
            "qa-platform-",
            "value",
            "platform/does-not-matter/kubeconfig",
    ...
    async fn a_readable_but_malformed_argo_kubeconfig_is_classified_not_echoed() {
        ...
        std::fs::write(&path, format!("-----BEGIN EC PRIVATE KEY----- {CANARY} -----END EC PRIVATE KEY-----\n"))
