...
//! ```
//!
//! which was persisted into `qa_platforms.version_detect_error`, published on
//! `PlatformDto` to every `qa.platform` GET/LIST-authorized caller, and
//! rendered on the platform page. serde quotes the offending scalar; for a
...
//!
//! [`describe_kube_error`] does interpolate `Status::message` and
//! `Status::code` for [`kube::Error::Api`]. That text is a string the
//! **API server sent back**, not anything derived from the material — and it
//! is the text legacy relies on ("namespaces virtuozzo not found" is what
//! makes a broken platform fixable rather than merely broken, per
//! `kube_observer`'s own header). Every other variant gets a fixed string,
//! because at least one of them — `AuthError::AuthExecRun`, reachable both
//! directly through [`kube::Error::Auth`] and boxed inside
...
//!
//! [`describe_kubeconfig_error`] matches every [`KubeconfigError`] variant by
...
//!
//! [`describe_kube_error`]'s non-`Api` catch-all used to be one bucket for
...
//! [`is_connect_failure`] downcast the error's cause chain to the *types*
//! `kube`'s own HTTP/TLS stack raises — [`rustls::Error`] and
//! [`hyper_util::client::legacy::Error`] — which are facts about the failure's
//! shape, the same category of fact [`describe_kubeconfig_error`] already
...
pub(super) fn describe_kubeconfig_error(error: &KubeconfigError) -> &'static str {
    match error {
        ...
        KubeconfigError::KindMismatch => {
            "its `kind` is not `Config`"
        }
        KubeconfigError::ApiVersionMismatch => {
            "its `apiVersion` is not `v1`"
        }
        ...
        KubeconfigError::ParseCertificates(_) => {
...
pub(super) const CLIENT_BUILD_FAILURE: &str =
    "its credentials could not be turned into a working Kubernetes client (check that \
...
pub(super) const INFER_FAILURE: &str =
    "no Kubernetes configuration could be inferred: this process is not running in-cluster \
...
const COULD_NOT_CONNECT: &str =
    "the API server could not be reached (connection timed out, was refused, or the address \
...
const TLS_CERTIFICATE_FAILURE: &str =
    "the connection to the API server failed a TLS certificate check; either the address the \
...
const OTHER_TRANSPORT_OR_CREDENTIAL_FAILURE: &str =
    "the request to the API server failed in a way that was neither a TLS/certificate problem \
...
pub(super) fn describe_kube_error(error: &kube::Error) -> String {
    match error {
...
mod tests {
    ...
    fn a_wrong_shape_failure_over_canary_bearing_input_is_classified_not_echoed() {
        let text = format!("-----BEGIN EC PRIVATE KEY-----\\n{CANARY}\\n-----END EC PRIVATE KEY-----");
        let error = kube::config::Kubeconfig::from_yaml(&text)
    ...
    fn an_api_status_keeps_the_servers_own_words() {
        ...
        assert!(described.contains("404"), "the status code must survive, got: {described}");
    }
    ...
    async fn a_refused_connection_is_classified_as_could_not_connect() {
        ...
        assert!(
            format!("{error:?}").contains("ConnectionRefused") || format!("{error:?}").contains("Connect"),
    ...
    async fn a_certificate_san_mismatch_is_classified_as_tls_certificate_failure() {
        ...
        let mut params = CertificateParams::new(vec!["not-the-loopback-address.invalid".to_owned()])
        ...
        let cert = params.self_signed(&key_pair).expect("self-signing the test cert");
        let ca_pem = cert.pem();
