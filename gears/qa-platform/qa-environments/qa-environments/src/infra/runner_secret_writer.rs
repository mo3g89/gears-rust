...
/// cluster and carries the current credential material, converging via
/// server-side apply so create, update and every self-heal cycle agree
/// rather than conflict.
///
/// `material` is whatever the environment stored under `credstore_ref` — an
/// SSH private key, a vinfra password, a kubeconfig. It was named `kubeconfig`
/// until 2026-09-09, which was true only while VHP was the sole product; the
/// function never read it as one.
///
/// # Errors
/// Every error names what failed (bad path, unreachable API server, RBAC,
/// non-existent namespace) and never the credential bytes themselves — the
...
pub(super) async fn ensure_runner_secret(
...
/// operator's side, not this writer's (see [`ensure_runner_secret`]'s own
...
mod tests {
    ...
    async fn a_missing_argo_kubeconfig_file_is_a_named_failure_not_a_panic() {
        let result = ensure_runner_secret(
    ...
    /// The canary sits directly in the credential bytes handed to
    /// `ensure_runner_secret`, unencoded, so an accidental
    ...
    #[tokio::test]
    async fn the_credential_material_never_reaches_an_error_message() {
        let material = format!(
        ...
        assert!(
            ...
            "the error string must never carry the credential material, got: {error}"
        );
    }
