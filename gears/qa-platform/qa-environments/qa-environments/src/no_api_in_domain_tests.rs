...
//!
//! The dependency runs the other way: `api` is a transport over `domain`. This
//! crate's live offender was a domain *test* module reaching for the REST
//! request and response DTOs; see [`ALLOWED`] for what was decided about it.
//! The rule itself is the subsystem's, installed in all four gears rather than
//! only where an import had already appeared. Review findings #15, #16, #39.
//!
...
//! shape qa-runs already uses for its other structural guards
//! (`qa-runs/src/doc_citations_tests.rs`, `file_citations_tests.rs`), and in
//! all four gears of the subsystem rather than only the ones that had an
//! offender — a gate installed in one crate leaves the other three unguarded.
//!
//! # What is checked
//!
...
///
/// # The one entry, and why it is an allowance rather than a fix
///
/// `environments_kubeconfig_tests` ends in
/// `the_document_never_reaches_a_log_line_a_debug_rendering_or_a_response_body`,
/// which drives `EnvironmentsService` over a real in-memory database and then
/// asserts one property across every channel a pasted private key could leave
/// by: the emitted log text, the `Debug` rendering of all ten types that can
/// carry the document, and `serde_json::to_string(&EnvironmentDto::from(..))`
/// -- the body a REST caller actually receives. Four of those ten renderings
/// are REST request DTOs and the body assertion is a REST response DTO, which
/// is the whole of the import.
///
/// Both fixes the rule prefers were considered and both weaken the test:
///
/// * **Converting to domain types** is not available. The DTO renderings *are*
///   the point -- `CredentialSubmissionDto` was added by the same task that
///   added the domain-side channel, and the file's own comment records that a
///   reviewer removed the DTO redaction and all 231 tests still passed because
///   this array had not been extended. Dropping them re-opens exactly that.
/// * **Relocating to `api/rest`** splits one property into two tests that can
///   each pass while the property fails, needs three file-local fixtures
///   (`pasted`, `services_with`, `sole_ref`) and five constants moved or
///   copied, and would put a second copy of `SECRET_FRAGMENT` -- whose value
///   this file's header declares appears nowhere else in the repository -- in
///   the tree.
///
/// The import is also in a `#[cfg(test)]` module, so the coupling the rule
/// exists to prevent -- a shipped domain artefact depending on the transport
/// layer -- does not occur here. That is why an allowance is proportionate for
/// this file and would not be for a production module.
const ALLOWED: &[(&str, &str)] = &[(
    "src/domain/service/environments_kubeconfig_tests.rs",
    "a whole-stack leak test: one assertion set proving a pasted kubeconfig \
     reaches neither a log line, nor the `Debug` of any type that can carry it \
     (domain models and REST DTOs alike), nor the serialized response body -- \
     see this module's `ALLOWED` doc for why splitting it weakens it",
)];
