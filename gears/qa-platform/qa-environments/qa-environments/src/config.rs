...
//! failure: the same config file that a `platform-observation` build accepts
...
//! build without `platform-observation` these fields are accepted and then
...
pub struct QaEnvironmentsConfig {
    ...
    /// How to reach the **Argo** cluster that `KubeObserver::ensure_kubeconfig_secret`
    /// (D4) writes the runner's kubeconfig `Secret` into — a different
    /// cluster from any platform's own. Only *meaningful* in a build carrying
    /// the `platform-observation` cargo feature — `crate::gear::build_observer`
    ...
    /// keeps every platform's detection fresh without anyone calling
    /// `POST /qa/v1/platforms/{id}/refresh`.
    ///
    /// Its *reader* is feature-gated for the same reason [`Self::argo`]'s is:
    /// with no `PlatformObserver` beyond
    /// [`crate::domain::ports::NoopObserver`], a ticker in a default build
    /// would have nothing to do but log a failure every cycle — `crate::gear`
    /// declines to spawn it at all in that build instead, and logs once at
    /// startup saying so. The field itself parses in every build; see this
...
pub struct ArgoObserverConfig {
    ...
    /// Namespace the runner's kubeconfig `Secret` is written into. Default
    /// `argo`, matching `qa-runs`' own default and
    ...
    /// `infra::observer::secret_writer`'s parity-oracle test module, which
    /// pins this string against `qa-runs`' `argo/naming.rs` and the
    /// provisioning script.
    pub secret_prefix: String,
    /// Key under which the kubeconfig bytes are stored in the `Secret`'s
...
pub struct ObservationConfig {
    ...
    /// in a build without `platform-observation`, logging once at startup to
...
impl ObservationConfig {
    ...
    pub fn effective_poll_interval_seconds(&self) -> u64 {
        ...
        self.poll_interval_seconds.max(Self::MIN_POLL_INTERVAL_SECONDS)
    }
}
...
mod observation_config_tests {
    ...
    /// `platform-observation`). Before 2026-08-28 the two fields it names
...
observation:
  ...
  poll_interval_seconds: 300
";
        let cfg: QaEnvironmentsConfig =
            serde_yaml::from_str(yaml).expect("an --argo deployment's config must parse in ANY build");
        assert_eq!(cfg.argo.kubeconfig_path.as_deref(), Some("/etc/qa-platform/k3s-kubeconfig.yaml"));
