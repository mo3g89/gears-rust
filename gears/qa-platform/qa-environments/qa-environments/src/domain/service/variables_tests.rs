...
use super::test_support::{MockPlatformsRepository, ctx, permissive_response, test_db_provider};
...
/// oracle regression test asserts this count is zero when the platform
...
) -> VariablesService<MockVariablesRepository, MockPlatformsRepository> {
...
async fn upsert_existing_row_requests_update_scope() {
    ...
    let svc = build_service(variables_repo.clone(), platforms_repo, authz.clone()).await;
...
/// `platform_id` (one the caller's platforms repo can't see) must 404 from
...
/// about a `platform_id` the caller isn't authorized to know about.
#[tokio::test]
async fn upsert_foreign_platform_is_not_found_before_probe() {
    let tenant_id = Uuid::new_v4();
    let foreign_platform_id = Uuid::new_v4();

    let variables_repo = Arc::new(MockVariablesRepository::empty());
    // The platforms mock has no platform registered, so any `get(id)`
    // returns `None` regardless of `id` — simulating a platform_id the
    ...
    let err = svc
        .upsert(
            &ctx(tenant_id),
            NewVariable {
                platform_id: Some(foreign_platform_id),
                name: "BAZ".to_owned(),
                value: "value".to_owned(),
            },
        )
    ...
    assert!(
        matches!(err, DomainError::PlatformNotFound { id } if id == foreign_platform_id),
        "expected PlatformNotFound, got {err:?}"
    );
    assert_eq!(
        variables_repo.find_call_count(),
        0,
        "the natural-key probe must never run before the platform precheck rejects a foreign platform_id"
    );
}
...
// this gear owns two of them (pipeline and platform variables, both reaching
...
/// Every reserved name is refused, on both the pipeline (`platform_id: None`)
/// and the per-platform path — the two paths this gear owns.
///
...
async fn upsert_refuses_every_reserved_name_on_both_paths() {
    ...
    for reserved in RESERVED_VARIABLE_NAMES {
        for scope in [None, Some(platform_id)] {
            let variables_repo = Arc::new(MockVariablesRepository::empty());
            let platforms_repo = Arc::new(MockPlatformsRepository::none());
            let authz = Arc::new(RecordingAuthZ::new());
            let svc = build_service(variables_repo.clone(), platforms_repo, authz).await;
...
async fn upsert_refuses_a_reserved_name_in_any_casing() {
    ...
    for spelling in ["rp_api_key", "Rp_Api_Key", "rP_aPi_KeY"] {
        ...
        let platforms_repo = Arc::new(MockPlatformsRepository::none());
        let authz = Arc::new(RecordingAuthZ::new());
        let svc = build_service(variables_repo, platforms_repo, authz).await;
