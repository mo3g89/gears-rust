...
/// Environment lease service.
#[domain_model]
pub struct LeasesService<L: LeasesRepository, P: EnvironmentsRepository> {
    db: Arc<DbProvider>,
    repo: Arc<L>,
...
impl<L: LeasesRepository, P: EnvironmentsRepository> LeasesService<L, P> {
    ...
    #[instrument(skip(self, ctx), fields(environment_id = %environment_id, run_id = %run_id))]
    pub async fn acquire(
        ...
        // Environment precheck: new acquisitions require the environment to exist
        // and be marked available. Existing holders are unaffected by an
        // environment going unavailable later — see doc comment above.
        ...
        if !environment.available {
            return Err(DomainError::EnvironmentUnavailable { id: environment_id });
        }
    ...
    /// Release `run_id`'s hold on `environment_id`. Idempotent: releasing a run
    /// that does not currently hold the lease leaves the state unchanged and
    /// succeeds. No environment-availability precheck: release must work even
    /// on environments that were flipped unavailable while held.
    #[instrument(skip(self, ctx), fields(environment_id = %environment_id, run_id = %run_id))]
    ...
    /// Read the current lease state for `environment_id`.
    ///
    /// Fails with `EnvironmentNotFound` for a nonexistent or foreign (wrong
    /// tenant) environment — without this precheck, a missing lease row (which
    /// is the normal, legitimate state for a never-leased environment) is
    /// indistinguishable from a missing environment, and both would silently
    /// read as `LeaseState::Free`. See `acquire`'s environment precheck above
    /// for the same rationale and the note on why this needs its own
    /// PEP-derived PLATFORM scope rather than reusing `scope`.
    #[instrument(skip(self, ctx), fields(environment_id = %environment_id))]
    pub async fn get(
        &self,
        ctx: &SecurityContext,
        environment_id: Uuid,
        ...
        let scope = self
            .policy_enforcer
            .access_scope(ctx, &resources::LEASE, actions::GET, Some(environment_id))
            .await?;
        ...
        let versioned = self.repo.get(&conn, &scope, environment_id).await?;
        Ok(versioned.state)
    }
}
