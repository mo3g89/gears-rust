//! `EnvironmentsService` tests for the `default_branch` override, against a REAL
...
use qa_product_sdk::observation::{
    FailureClass, HealthOutcome as PluginHealthOutcome, HealthState,
    ObservationOutcome as PluginObservationOutcome, ObservedAttrs, PluginFailure,
    PluginObservation,
};

use crate::domain::error::DomainError;
use crate::test_support::{
    FixedPluginPort, SequencedPlugin, build_services_tenant_scoped,
    build_services_tenant_scoped_with_plugin, ctx, inmem_db,
};
use uuid::Uuid;

fn new_environment(name: &str, default_branch: Option<&str>) -> NewEnvironment {
    NewEnvironment {
...
/// through the service, read back through a *separate* `get_environment` call so
...
async fn a_default_branch_set_at_create_survives_the_round_trip() {
    ...
    let created = services
        .environments
        .create_environment(&ctx(tenant), new_environment("pinned", Some("release-9.0")))
        .await
        .unwrap();
...
#[tokio::test]
async fn an_environment_created_without_an_override_reads_back_as_none() {
    let db = inmem_db().await;
    let services = build_services_tenant_scoped(db);
    let tenant = Uuid::new_v4();
    ...
    let created = services
        .environments
        .create_environment(&ctx(tenant), new_environment("unpinned", None))
        .await
        .unwrap();
...
/// See `resolve_branch`'s "the environment tier's filter is redundant, and stays"
...
async fn the_create_path_trims_the_override_and_treats_blank_as_absent() {
    ...
    let trimmed = services
        ...
        .create_environment(
            &ctx(tenant),
            new_environment("padded", Some("  release-9.0\n")),
        )
        .await
        .unwrap();
    ...
    for (name, blank) in [("blank-empty", ""), ("blank-spaces", "   ")] {
        let created = services
            .environments
            .create_environment(&ctx(tenant), new_environment(name, Some(blank)))
            .await
            .unwrap();
        assert_eq!(
...
async fn an_override_wider_than_the_column_is_a_validation_error_on_both_paths() {
    ...
    let ok = services
        .environments
        .create_environment(&ctx(tenant), new_environment("at-limit", Some(&at_limit)))
        .await
        .unwrap();
    ...
    let create_err = services
        .environments
        .create_environment(&ctx(tenant), new_environment("over", Some(&over_limit)))
        .await
        .unwrap_err();
    assert!(
        matches!(
...
async fn a_multibyte_override_is_measured_in_characters_not_bytes() {
    ...
    let created = services
        .environments
        .create_environment(&ctx(tenant), new_environment("cyrillic", Some(&cyrillic)))
        .await
    ...
    let err = services
        ...
        .create_environment(
            &ctx(tenant),
            new_environment("cyrillic-over", Some(&too_many)),
        )
        .await
        .unwrap_err();
    assert!(
...
/// The three states of `EnvironmentPatch::default_branch`, which is the reason it
...
/// unpinning an environment would find the old branch still in force with no error
...
async fn the_patch_path_distinguishes_absent_from_cleared_from_set() {
    ...
    let created = services
        ...
        .create_environment(
            &ctx(tenant),
            new_environment("tri-state", Some("release-9.0")),
        )
        .await
        .unwrap();
...
/// `{"default_branch": ""}` actually unpins the environment.
#[tokio::test]
async fn a_patch_with_a_blank_value_clears_the_override_rather_than_storing_it() {
    ...
    let created = services
        ...
        .create_environment(
            &ctx(tenant),
            new_environment("blank-patch", Some("release-9.0")),
        )
        .await
        .unwrap();
...
///
/// # Rewritten by Task 15, and what it no longer asserts
///
/// Observation goes through the product plugin now, so the outcomes are
/// scripted on a [`SequencedPlugin`] rather than on a `PlatformObserver`
/// double (a port Task 19 has since deleted). Two assertions had to go with that change, and neither was
/// dropped quietly:
///
...
///   clearing.
/// * the derived counts, for the same reason.
///
/// What this test is *for* is untouched: the three states stay three states,
/// and the new `health_state` column is asserted alongside the legacy view so
...
async fn never_checked_unreachable_and_healthy_are_three_distinct_states() {
    ...
    let plugin = Arc::new(SequencedPlugin::new([
        PluginObservation {
            environment: PluginObservationOutcome::Failed(
                PluginFailure::classified(FailureClass::NotFound, "the target could not be read")
                    .with_remote_message("namespaces not found"),
            ),
            health: PluginHealthOutcome::Failed(
                PluginFailure::classified(
                    FailureClass::Unreachable,
                    "the target could not be reached",
                )
                .with_remote_message("the API server could not be reached"),
            ),
        },
        PluginObservation {
            environment: PluginObservationOutcome::Detected({
                let mut attrs = ObservedAttrs::default();
                attrs.set("platformVersion", "26.5");
                attrs.set("build", "0");
                attrs.set("namespace", "virtuozzo");
                attrs
            }),
            health: PluginHealthOutcome::Checked {
    ...
    // A pasted kubeconfig, not a bare reference: `observe_environment`
    // resolves the reference back through credstore before it ever reaches the
    // plugin, and only a paste (which this gear writes to credstore itself via
    // `RecordingCredStore`) is guaranteed to resolve in this harness.
    let created = services
        .environments
        .create_environment(
            &ctx(tenant),
            NewEnvironment {
                ...
                credentials: std::collections::BTreeMap::new(),
                name: "p1".to_owned(),
                // A product, so a plugin can be resolved at all.
                product_id: Uuid::from_u128(0x9001),
                description: None,
                kubeconfig: Some(CredentialMaterial::new(
                    "apiVersion: v1\nkind: Config\n".to_owned(),
                )),
                default_branch: None,
                is_default: false,
        ...
        .unwrap();
    assert_eq!(
        created.health_checked_at, None,
        "an environment no cycle has reached has never been looked at"
    );
    ...
    let unreachable = services
        ...
        .unwrap();
    assert_eq!(
        unreachable.health_detail.as_deref(),
        Some("the target could not be reached: the API server could not be reached"),
        "the classified reason reaches the operator-facing column"
    );
    assert_eq!(
        unreachable.health_state,
        HealthState::Unknown,
        "`unknown`, not `down`: the read failed, so nothing is known about the \
         target -- the same distinction the legacy pair draws between \
         Unreachable and Unhealthy"
    );
    assert!(unreachable.health_checked_at.is_some());
    ...
    let healthy = services
        ...
        .await
        .unwrap();
    assert_eq!(healthy.health_state, HealthState::Ok);
    assert_eq!(
        healthy.health_detail.as_deref(),
        Some("Healthy"),
        "the plugin's own classified word reaches the operator-facing column; \
         Task 19 dropped the legacy `cluster_status` that used to carry it"
    );
    assert_eq!(
        healthy.observed_version.as_deref(),
        Some("26.5"),
        "and the environment half wrote its own columns in the same call"
    );
}
