...
fn every_resource_descriptor_carries_its_own_name_const() {
    assert_eq!(
        ...
        "resources::PLATFORM was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::VARIABLE.name(),
        resources::VARIABLE_NAME,
        "resources::VARIABLE was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::LEASE.name(),
        resources::LEASE_NAME,
        "resources::LEASE was built from the wrong *_NAME const",
    );
}
