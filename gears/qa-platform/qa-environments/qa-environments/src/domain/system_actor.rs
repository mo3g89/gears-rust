...
//! per-environment work — resolving an environment's kubeconfig out of credstore,
...
//! [`crate::domain::repos::EnvironmentsRepository::list_all_with_tenant`] does
//! take a `scope: &AccessScope` (`domain/repos/environments_repo.rs`) — it is
...
//! (`domain/service/environments.rs`, inside the block-comment exception that
...
//! repository method's own doc gives: "which environments exist" is this gear's
...
/// The observation ticker's per-environment context, bound to the tenant that
/// owns the environment being observed.
///
/// `tenant_id` must be non-nil — it comes from a stored
/// `qa_environments.tenant_id` column via
/// [`crate::domain::repos::EnvironmentsRepository::list_all_with_tenant`], and
