...
//!
//! A caller may override any one field's filler via [`Canary::with_config`].
//! This exists for a plugin whose non-secret configuration names a
//! *connection target* rather than an inert label — VHI's `node_host` and
//! `ssh_port`, which a generic `"canary-field-node_host"` can never resolve
//! or connect to. Without it, [`assert_no_leak`] cannot reach such a
//! plugin's success path at all: every drive stops at whatever failure
//! "cannot reach a host named `canary-field-node_host`" classifies as, which
//! proves nothing about the code that handles credential material once a
//! session actually opens. A plugin whose target lives *inside* a
//! `MultilineSecret` field instead (VHP's `kubeconfig`, which carries its own
//! API server address) has never needed this — planting a real, working
//! document under [`Canary::pem`] was always enough.
//!
...
pub struct Canary {
    ...
    pub password: String,
    /// Values [`plant`] plants under a declared **non-secret** field's key,
    /// instead of that function's own generic `filler_for(key)` — see
    /// [`Self::with_config`].
...
impl Canary {
    ...
    /// Steer the value [`plant`] writes under `key` in the synthesised
    /// environment's non-secret `config`, instead of this module's own
    /// generic `filler_for(key)`.
    ///
    /// Additive and empty by default: a caller that never calls this sees
    /// exactly the filler behaviour every existing caller already gets. It
    /// exists because a generic filler is sometimes not merely un-realistic
    /// but actively wrong — a plugin whose non-secret configuration names a
    /// connection target (an SSH host, a port) can never reach its own
    /// success path against `"canary-field-<key>"`, so a drive that needs to
    /// prove that path is reachable has to be able to say what a specific
    /// declared key should be instead. Has no effect on a field
    /// [`Canary::marker_for`] plants a secret marker under: this only ever
    /// replaces a *filler*.
    #[must_use]
...
///
/// A non-secret field's value is [`Canary::with_config`]'s override when the
/// caller supplied one for that key, and [`filler_for`] otherwise — so a
/// plugin whose configuration names a connection target can be driven all
/// the way to a real success path by a caller that knows what that target
/// should be, while every other field, and every caller that never calls
/// `with_config`, keeps the original generic filler.
    ...
    for field in schema {
        let value = if let Some(marker) = canary.marker_for(field.kind) {
            ...
            let filler = canary
                ...
                .get(&field.key)
                .cloned()
                .unwrap_or_else(|| filler_for(&field.key));
