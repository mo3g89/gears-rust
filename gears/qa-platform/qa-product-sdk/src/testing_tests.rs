...
/// [`Canary::with_config`]'s whole reason to exist: a plugin whose non-secret
/// configuration names a connection target (an SSH host, a port) can never
/// reach a real success path against a generic `canary-field-<key>` filler,
/// so an override on a **non-secret** declared field must actually replace
/// that filler rather than merely accompany it.
#[test]
fn a_config_override_replaces_the_generic_filler_for_a_non_secret_field() {
    let canary = Canary::vhp_shaped().with_config(DECLARED_CONFIG_KEY, "steered-value");
    let planted = plant(&vhp_shaped_credential_schema(), &canary);
...
/// An override named after a **secret**-kind declared field is silently
/// ignored: `plant` only ever consults [`Canary::with_config`]'s overrides in
/// the non-secret branch, so there is no way for a caller to use it to
/// displace the marker a `MultilineSecret`/`Secret` field is planted with.
///
/// That silent ignore is deliberate, and pinned here rather than left
/// implicit: an override that *could* shadow a marker would hollow out this
/// harness's own leak detection for every caller that ever called
/// `with_config` on a key that happened to also be a secret one — silently,
/// with every other assertion in a run still green, which is the exact
/// failure mode this whole harness exists to catch.
#[test]
fn a_config_override_named_after_a_secret_field_is_ignored_and_its_marker_still_wins() {
    let canary = Canary::vhp_shaped().with_config(DECLARED_SECRET_KEY, "not-the-marker");
    let planted = plant(&vhp_shaped_credential_schema(), &canary);
    ...
    assert_eq!(
        submitted.as_deref(),
        Some(canary.pem.as_str()),
        "an override must not be able to displace a secret field's marker"
    );
}
