...
- **Legacy fields the fourth surface needed:** `PlatformInfo.version_detect_error` and
  `version_detected_at`, i.e. a version-detection cycle. `PlatformDto` had
...
- [2026-08-28 cluster health](../docs/superpowers/specs/2026-08-28-cluster-health-design.md)
  ...
  and `cluster_checked_at`. `PlatformDto` carries them as one optional `cluster` object.
...
| Platform detail: the refresh button | restored and **enabled** — `useRefreshPlatform` calls `POST /qa/v1/platforms/{id}/refresh`, which re-observes live |
