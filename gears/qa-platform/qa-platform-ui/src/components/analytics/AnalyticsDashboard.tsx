...
import { useAnalyticsProductSelection } from '@/lib/analyticsProductSeed';
  ...
  return (
    ...
    {runTarget && (
      <RunTestDialog
        ...
        initialEnvironment={runTarget.last_platform ?? undefined}
      />
    )}
    </>
  );
...
export function AnalyticsDashboard({ scope, planId }: AnalyticsDashboardProps) {
  ...
  // nothing. `useAnalyticsProductSelection` carries why the deep link is
  // captured at mount rather than read live, and why the selector below can
  // still be pointed somewhere else without being snapped back.
  const [selectedProductId, setSelectedProductId] = useAnalyticsProductSelection(
  ...
  useEffect(() => {
    ...
    // **First product, not VHP.** This preferred a product whose key is
    // literally "VHP", which put one product's name in the code path every
    // other product's dashboard takes -- and silently ranked them. The
    // remembered selection wins above; otherwise the first product does, which
    // is the same result in a single-product deployment and a defensible one
    // in any other.
    const [preferred] = products;
    if (preferred) {
      setSelectedProductId(preferred.id);
    }
    // `setSelectedProductId` is now the setter `useAnalyticsProductSelection`
    // returns rather than a bare `useState` one, so it has to be declared —
    // eslint can no longer see that it is a stable `useState` setter. It is
    // one: the hook returns it unwrapped.
