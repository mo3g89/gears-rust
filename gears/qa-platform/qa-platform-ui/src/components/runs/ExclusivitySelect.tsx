...
export function ExclusivitySelect({
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor={id}>Environment access</Label>
      ...
      <p className="text-xs text-muted-foreground">
        Auto follows plan.yaml and each test's TEST_META. Exclusive waits until the environment is
        free, then holds it alone — anything launched meanwhile waits behind it. Either way a run
        is never rejected for a busy environment: it queues and starts automatically.
      </p>
    </div>
  );
}
