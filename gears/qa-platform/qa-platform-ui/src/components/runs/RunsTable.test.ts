...
describe('RunsTable — the succeeded-with-skips marker', () => {
  it('renders for a run whose real (lowercase) phase is succeeded and skipped > 0', () => {
    renderTable([
      run({ phase: 'succeeded', result: { passed: 10, failed: 0, skipped: 5, in_progress: 0, total: 15 } }),
    ]);
  ...
  it('does not render for a succeeded run with no skips', () => {
    renderTable([
      run({ phase: 'succeeded', result: { passed: 10, failed: 0, skipped: 0, in_progress: 0, total: 10 } }),
    ]);
  ...
  it('does not render for a failed run, even with a non-zero skip count', () => {
    ...
    renderTable([
      run({ phase: 'failed', result: { passed: 9, failed: 1, skipped: 5, in_progress: 0, total: 15 } }),
    ]);
    expect(screen.queryByText(/skipped$/)).toBeNull();
  });
});
