...
interface RunsTableProps {
  runs: WorkflowRun[];
  environmentBuildByName?: Map<string, string>;
              ...
              >
                ...
                <TableCell className="tabular-nums">
                  {(() => {
                    ...
                    const fallbackBuild =
                      (run.platform ? environmentBuildByName?.get(run.platform)?.trim() : '') || '';
                    const effectiveBuild = snapshotBuild || fallbackBuild;
                    // Display format matches vpadm: "<version>.<build>" (the
                    // Environments table renders the same way) instead of the
