...
interface QueuedRunsCardProps {
  /** Restrict to one environment. Omit for every environment, and the column appears. */
  environment?: string;
}

export function QueuedRunsCard({ environment }: QueuedRunsCardProps) {
  const { data, isLoading } = useRunQueue(environment, REFETCH_MS);
  ...
  const handleForceStart = async (entry: RunQueueEntry) => {
    const ok = await confirm({
      ...
      description:
        'This starts the run now, ignoring what is already running on the environment — ' +
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          {environment ? ` on ${environment}` : ' across all environments'}. Each starts automatically
          once its environment is free.
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {!environment && <TableHead>Environment</TableHead>}
            ...
            <TableBody>
              {queued.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  {!environment && <TableCell>{entry.platform}</TableCell>}
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        title="Start now, ignoring environment occupancy"
                      >
                        {busyId === entry.id ? (
