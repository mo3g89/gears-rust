...
import { EnvironmentInfo, ScheduleInfo } from '@/api/types';
...
export function CreateScheduleDialog({
  ...
  const [environment, setEnvironment] = useState(editSchedule?.platform || '');
  ...
  useEffect(() => {
    if (open && editSchedule) {
      ...
      setEnvironment(editSchedule.platform || '');
  ...
  const requiresEnvironment = isTestSchedule || isCustomPlan;
  ...
  const selectedEnvironmentInfo = useMemo(
    () => environmentList.find((p) => p.name === environment) ?? null,
    [environmentList, environment]
  );

  // Same "Default cluster" resolution the run dialog uses — see
  // `defaultEnvironmentForProduct`. A schedule that fires against an unintended environment
  // is worse than a manual run doing so: nobody is watching when it happens.
  const defaultEnvironment = useMemo(
    () => defaultEnvironmentForProduct(environmentList, productId ?? null),
    [environmentList, productId]
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const effectiveEnvironment = environment || (requiresEnvironment ? '' : defaultEnvironment?.name) || '';
    if (!effectiveEnvironment) {
      toast.error('Please select an environment', {
        description: requiresEnvironment
          ? undefined
          : 'This product has no single default environment, so "Default cluster" cannot be resolved.',
      });
      return;
    }
    ...
    <DialogContent>
      <form onSubmit={handleSubmit}>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="environment">{requiresEnvironment ? 'Environment' : 'Environment (Optional)'}</Label>
            {environmentList.length > 0 ? (
              <Combobox
                ...
                options={[
                  ...
                  ...environmentList.map((item) => ({ value: item.name, label: formatEnvironmentLabel(item) })),
                ]}
                placeholder={
                  requiresEnvironment ? 'Select environment' : defaultEnvironmentLabel(defaultEnvironment)
                }
              />
            ) : (
              <p className="text-sm text-muted-foreground">
                {requiresEnvironment
                  ...
                  : 'No environments configured. Add one before scheduling.'}
              </p>
            )}
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              !planId ||
              !cronExpr ||
              (requiresEnvironment && !environment) ||
