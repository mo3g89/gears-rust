...
export interface DashboardStats {
  ...
  /** Optional: qa-environments observes nothing about the cluster behind a platform, so
...
/** UI-only view model. `DashboardStatsDto` has no platform-health rollup at all — qa-environments
 *  models a platform's availability as a single `available: boolean` and observes nothing about
...
/** REST DTO shape for one node, as the platform's own cluster reported it —
 *  `qa-environments/src/api/rest/dto.rs`'s `NodeSummaryDto`, field for field. Hand-written
 *  rather than aliased to `S['NodeSummaryDto']`: the generated schema has no such export at
 *  all yet (`generated/openapi.d.ts` predates qa-environments' cluster-health commits —
 *  the dev server behind `npm run gen:api` has not been rebuilt since, confirmed live
 *  against `:8087/openapi.json`, which still lacks `ClusterHealthDto`/`NodeSummaryDto`/
 *  `NodeCountsDto` entirely). Regenerate and alias this once the generated file catches up. */
...
/** Node counts computed **server-side, once**, from `ClusterHealth.nodes` —
 *  `NodeCountsDto`'s own doc says "the UI renders these; it must never re-derive them",
 *  and nothing here does. Hand-written for the same reason `NodeSummary` above is. */
...
/** A platform's most recent cluster-health reading — `PlatformDto.cluster`, one optional
 *  object rather than a set of flat nullable fields, deliberately:
 *
 *  - `null` means no observation cycle has ever reached this platform yet.
 *  - A populated value with `status === "Unreachable"` means a cycle DID reach it and
 *    could not read the cluster — a different fact from `null`. `status_message` is set
 *    only in this case; `nodes` is `[]` and every `counts` field is `0` as an **artefact**
 *    of the empty node list, not a measurement, so a renderer must not show them beside
 *    an `Unreachable` status as if they were.
 *  - Any other `status` ("Healthy" | "Degraded" | "Unhealthy" | "Warning") means the read
 *    succeeded and `nodes`/`counts` are real.
 *
 *  Hand-written for the same reason `NodeSummary` above is — see that comment. Field for
 *  field identical to `qa-environments/src/api/rest/dto.rs`'s `ClusterHealthDto`, read
 *  directly since the generated schema has no export for it yet. */
...
/** The shape Task 10's adapter returns to components. `PlatformDto` renames
 *  `version`→`observed_version`, `build`→`observed_build` (CONTRACT-DIFF row 50), and its
 *  own namespace field is `observed_namespace`, not `namespace` — the platform detected
 *  its core components in that namespace, this UI never assigns one. `vhp_base_url`,
 *  `version_detected_at` and `version_detect_error` are served directly (Task 7/9).
 *  `cluster` is Task 5's addition to `PlatformDto` (`dto.rs:75`) — see `ClusterHealth`
 *  above for what its presence and its `status` mean. */
export interface PlatformInfo {
  /** Stable UUID assigned at platform creation (Phase A). Use `name` for
   ...
   *  platform per product has it set; see `defaultPlatformForProduct`. */
  is_default: boolean;
  /** Per-platform default branch used by Run/Schedule dialogs and the
   *  Platform Details page when listing plans/tests. Falls back to the
   *  repository's default branch when empty. */
  default_branch: string | null;
  /** This platform's most recent cluster-health reading, or `null` if no observation
   *  cycle has reached it yet. See `ClusterHealth` above. */
  cluster: ClusterHealth | null;
}

/** The shape Task 10's adapter returns to components for the platform detail page.
 *  Was, until Task 6, a hand-duplicated copy of `PlatformInfo` plus a set of dead optional
 *  cluster-shaped fields (`status?`, `node_count?`, …) that qa-environments had no source
 *  for and nothing here ever populated (CONTRACT-DIFF row 51, §8-C3). qa-environments now
 *  observes the cluster (Task 5), so those stand-ins are gone in favour of the same real
 *  `cluster` field `PlatformInfo` carries. */
export interface PlatformDetails {
  ...
  version_detect_error: string | null;
  default_branch: string | null;
  /** Whether this platform is its product's default; see `PlatformInfo.is_default`.
   *  Carried here so the Platform Details page can hand it to the edit dialog. */
  is_default: boolean;
  cluster: ClusterHealth | null;
}

/** The shape the Create Platform form builds. `CreatePlatformReq` now takes **either** a
...
export interface CreatePlatformForm {
  name: string;
  /** A pasted kubeconfig document, or a credstore reference to one already registered —
   *  `createPlatformReqFromForm` tells them apart, after undoing the dialog's `btoa`. */
  kubeconfig: string;
  description?: string;
  product_id?: string;
  namespace?: string;
  ...
  /** Create this platform as its product's default, demoting the previous holder.
   *  Absent means `false`. */
  is_default?: boolean;
}

/** The shape the Edit Platform form builds. The gear's `UpdatePlatformReq` is a `PATCH`
 *  (legacy was `PUT`) and has no `namespace` or `vhp_base_url` (CONTRACT-DIFF row 53). */
export interface UpdatePlatformForm {
  /** A replacement kubeconfig document, or a credstore reference to one. Absent leaves the
   *  stored kubeconfig alone. No component sets this today (the Edit Platform dialog has
  ...
  vhp_base_url?: string;
  default_branch?: string;
  /** Make this platform its product's default, or clear the flag. Absent leaves it
   *  unchanged — see `defaultPlatformForProduct` for what the flag drives. */
  is_default?: boolean;
}
...
export type AnalyticsGroupBy = 'none' | 'component' | 'tag' | 'platform';
 ...
 *  is `{id, platform_id, name, value}` and carries no `secure` field, and neither does the model
