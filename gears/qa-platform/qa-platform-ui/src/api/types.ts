...
// renamed, reshaped, or dropped outright (see `gears/qa-platform/docs/DESIGN.md` §3.6), or
...
export interface ScheduleInfo {
  ...
  schedule_id: string;
  plan_id: string;
  /** Present whenever the target names a repository (plan/test/collect); absent for a
   *  `custom_plan` target, whose product is instead resolved through its tests
   *  (`lib/productScope.ts`). Mirrors `WorkflowRun.repo_id` — both satisfy
   *  `ProductScopedRow` without a special case. */
  repo_id: string | null;
...
export interface DashboardStats {
  ...
  /** How many runs a `product_id` filter dropped because they could not be attributed to
   *  any product (a custom-plan run), as opposed to belonging to a different one. Always 0
   *  when no product filter was applied. `ProductScopeNotice` renders exactly when this is
   *  non-zero. */
