...
// calls into here.
//
// The field-level justification for each transform is `gears/qa-platform/docs/CONTRACT-DIFF.md`,
 ...
 *
 * **A null `platform_id` is passed through, not refused.**
 *
 ...
 * it is dispatched. `PRD.md:366` states the same intent: *"Runs without a target
 ...
 * the run itself.
 */
export function runFromDto(dto: S['RunDto']): WorkflowRun {
  return {
    name: dto.name,
    ...
    // X1: legacy drew a platform *name* here. `RunDto` carries only the uuid and
    // resolving it is a qa-environments lookup; `hooks.ts` does that where it already
    // holds the platform list, and leaves the uuid otherwise.
    platform: dto.platform_id ?? null,
    product_key: null,
...
/** Replace each run's `platform` uuid with the platform's name, where the name is known.
 *  X1 in the one direction the UI actually renders. A uuid with no matching platform is
 *  left as-is rather than blanked — an unresolvable id is still an identifier. */
export function withPlatformNames(runs: WorkflowRun[], platformNameById: Map<string, string>): WorkflowRun[] {
  return runs.map((run) =>
    run.platform && platformNameById.has(run.platform)
      ? { ...run, platform: platformNameById.get(run.platform)! }
      : run
  );
}
 ...
 */
export function queueEntryFromDto(dto: S['QueueEntryDto']): RunQueueEntry {
  return {
    id: dto.id,
    platform: dto.platform_id,
...
export function dashboardRunFromDto(dto: S['DashboardRunDto']): WorkflowRun {
  return {
    name: dto.name,
...
function failedCardFromDto(dto: S['FailedTestCardDto']): FailedTestCard {
  return {
 ...
 * carried.
 */
export function dashboardFromDto(dto: S['DashboardStatsDto']): DashboardStats {
  return {
 ...
 *  DTO. */
export function testRunResultFromDto(dto: S['TestResultDto']): TestRunResult {
  return {
    workflow_name: dto.run_id,
    plan_id: dto.repo_id ? encodePlanId(dto.repo_id, dto.plan_path ?? '') : (dto.plan_path ?? ''),
    phase: null,
 ...
 */
export function scheduleFromDto(dto: S['ScheduleDto']): ScheduleInfo {
  return {
    name: dto.name,
    schedule_id: dto.id,
    plan_id: planIdFromTarget(dto.target),
 ...
 *
 * `enabled` is **required and explicit** on every call. `qa-runs/qa-runs/src/api/rest/dto.rs:921-923` explains why an
 ...
 *
 * A **null `platform_id` is passed through**, for the same reason `launchReqFromForm`
...
// ---------------------------------------------------------------------------
// Platforms (CONTRACT-DIFF rows 50-56)
// ---------------------------------------------------------------------------

/** `ClusterHealthDto` (the wire shape described above) -> `ClusterHealth`. `null` passes
 *  straight through — "no observation cycle has reached this platform yet" is exactly
 *  what an absent `cluster` means on both sides (see `ClusterHealth`'s doc in `types.ts`). */
function clusterHealthFromDto(cluster: S['PlatformDto']['cluster']): ClusterHealth | null {
  if (!cluster) {
    return null;
  }
  return {
    status: cluster.status,
    status_message: cluster.status_message ?? null,
    nodes: (cluster.nodes ?? []).map((node) => ({
      name: node.name,
      control_plane: node.control_plane,
      ready: node.ready,
      kubelet_version: node.kubelet_version ?? null,
      os_image: node.os_image ?? null,
    })),
    namespace_count: cluster.namespace_count ?? null,
    // Passed through, not recomputed: `NodeCountsDto`'s own doc says the UI must never
    // re-derive these from `nodes` (see `NodeCounts` in `types.ts`).
    counts: cluster.counts,
    checked_at: cluster.checked_at,
  };
}

/** `PlatformDto` -> `PlatformInfo`. `observed_version`/`observed_build` are the renames
 *  §6.3 category 2 is about; `namespace` on this view model is `dto.observed_namespace` —
 *  what the platform's core components were actually detected in, not a name this UI
 *  invents. `vhp_base_url`, `version_detected_at` and `version_detect_error` are served
 *  directly (Task 7/9's four-field addition to `PlatformDto`). `cluster` is Task 5's
 *  addition, mapped by `clusterHealthFromDto` above. `available` and
 *  `kubeconfig_credstore_ref` have no legacy field and are not carried. */
export function platformFromDto(dto: S['PlatformDto']): PlatformInfo {
  return {
    id: dto.id,
    name: dto.name,
    created_at: dto.created_at,
    ...
    namespace: dto.observed_namespace ?? null,
    vhp_base_url: dto.vhp_base_url ?? null,
    ...
    cluster: clusterHealthFromDto(dto.cluster),
  };
}

/**
 * `PlatformDto` -> `PlatformDetails`.
 *
 * Was, before Task 5, a genuine partial substitute (§7.11): the cluster half — `status`,
 * `status_message`, six node counts, `namespace_count`, `checked_at`, `nodes[]` — had no
 * source anywhere in the gears, so it was left off entirely rather than filled with a `0`
 * or an `"unknown"` (§8-C3). qa-environments now observes the cluster and serves it as
 * `PlatformDto.cluster`, and `platformFromDto` already maps it, so this is a straight
 * reuse rather than a degraded one.
 */
export function platformDetailsFromDto(dto: S['PlatformDto']): PlatformDetails {
  return platformFromDto(dto);
}

// `qa_platforms.kubeconfig_credstore_ref` is `varchar(1024)` (gears migration
// `m20260814_000006_platform_default_branch.rs:58`). Nothing longer than that can ever
// be stored, whatever it contains, so a single-line value over this length cannot be a
// reference the gear could have saved -- the bound comes from the schema, not a guess
// about reference shape.
const MAX_CREDSTORE_REF_LENGTH = 1024;

/** `atob` the value if -- and only if -- it is shaped like base64; `null` otherwise.
 *
 *  `CreatePlatformDialog.tsx:47` calls `btoa(kubeconfig.trim())` on whatever was pasted
 *  before any adapter sees it, so decoding first is how this layer gets back to what the
 *  user actually typed. That component is copied verbatim and is not ours to change, which
 *  is why the decode lives here.
 *
 ...
 *  non-empty, inside the column's `varchar(1024)`, and printable ASCII.
 *
 ...
 *  first and persisting only the generated reference (`PlatformsService::create_platform`,
...
/** `CreatePlatformForm` -> `CreatePlatformReq`. The X7 rename is a **forward** now, not a
 *  refusal: see `kubeconfigFieldsFromFormValue`. `namespace`/`vhp_base_url` have no gear
 *  field and are dropped (CONTRACT-DIFF row 52). */
export function createPlatformReqFromForm(form: CreatePlatformForm): S['CreatePlatformReq'] {
  return {
    name: form.name,
    ...kubeconfigFieldsFromFormValue(form.kubeconfig),
...
/** `UpdatePlatformForm` -> `UpdatePlatformReq`. A true partial (`PATCH`), so only the keys
...
export function updatePlatformReqFromForm(form: UpdatePlatformForm): S['UpdatePlatformReq'] {
  const req: S['UpdatePlatformReq'] = {};
  ...
  // product's previous default" and `Some(false)` as "clear it on this platform".
  ...
  // Platform dialog has no kubeconfig field -- but the gear's PATCH accepts a pasted
...
/** One platform's observed version, with its build alongside. */
export interface ObservedPlatformVersion {
  platform_id: string;
  version: string;
  build: string | null;
}
...
/**
 * Derive observed versions from the platform DTO's `observed_version`/`observed_build`.
 *
 * A platform with a null `observed_version` is **dropped**, not rendered as `"null"` or
 ...
 * instead (§7.10).
 */
export function observedFromPlatforms(
  platforms: Array<{ id: string; observed_version?: string | null; observed_build?: string | null }>
): ObservedPlatformVersion[] {
  const out: ObservedPlatformVersion[] = [];
  for (const platform of platforms) {
    const version = platform.observed_version?.trim();
    if (!version) {
      continue;
    }
    out.push({ platform_id: platform.id, version, build: platform.observed_build ?? null });
  }
  return out;
}
...
export function distinctObservedVersions(
  ...
  return [...new Set(observedFromPlatforms(platforms).map((entry) => entry.version))].sort((a, b) =>
    a.localeCompare(b)
  );
}
...
export function productFromDto(dto: S['ProductDto']): Product {
  return {
    id: dto.id,
    name: dto.name,
    key: dto.key,
...
export function productReqFromForm(form: CreateProductForm): S['CreateProductReq'] {
  return {
    name: form.name,
    key: form.key,
 ...
 *  gear keeps `last_platform` as a name *and* adds `last_platform_id`, so the name is
 *  used and the uuid dropped. The read is bounded to the trailing 90 days where legacy
 *  had no window (row 67) — a semantic difference no adapter can undo. */
export function testAnalyticsFromDto(dto: S['PlanTestAnalyticsDto']): TestAnalytics {
  return {
    test_name: dto.test_name,
    last_platform: dto.last_platform ?? null,
...
function analyticsListItemFromDto(dto: S['AnalyticsListItemDto']): AnalyticsListItem {
  return {
    test_file: dto.test_file,
    test_name: dto.test_name,
    component: dto.component ?? null,
 ...
 *  - `grouped.platform[]` carries `platform` (a nullable *name*) plus `platform_id`,
 *    where `grouped.component`/`.tag` keep a bare `value`. The three are one chart in the
 *    UI, so the platform arm is projected onto `value` using the name, falling back to
 *    the uuid when the name is null — an unresolved platform is still identifiable, and
 *    blanking it would silently merge every unnamed platform into one bar.
 *
...
export function analyticsOverviewFromDto(
  ...
  return {
    ...
    grouped: {
      ...
      tag: dto.grouped?.tag ?? [],
      platform: (dto.grouped?.platform ?? []).map((row) => ({
        value: row.platform ?? row.platform_id,
        total: row.total,
        passed: row.passed,
        failed: row.failed,
        not_run: row.not_run,
 ...
 *  string now; `(repo_id, plan_path)` folds into `plan_id` and `platform_id` into
 *  `platform` (row 97). */
export function jiraBugFromDto(dto: S['JiraBugDto']): JiraBug {
  return {
    id: dto.id,
 ...
 *
 * `GET /qa/v1/variables?platform_id=<uuid>` is **additive**, not a filter: its own doc
 * says "List global pipeline variables, **plus** a platform's variables when
 * `platform_id` is given", and that is what it does. Driven live during this task with
 * three seeded rows:
 *
 *     (no platform_id)   -> [T10_GLOBAL]
 *     ?platform_id=<p1>  -> [T10_GLOBAL, T10_P1]
 *     ?platform_id=<p2>  -> [T10_GLOBAL, T10_P2]
 *     ?platform_id=<unknown uuid> -> 404 "Platform … was not found"
 *     ?platform_id=not-a-uuid     -> 400
 *
 ...
 */
export function partitionPlatformVariables(
  rows: S['VariableDto'][],
  platformId: string | null
): PipelineVariable[] {
  return rows
    .filter((row) => (platformId ? row.platform_id === platformId : !row.platform_id))
...
export function variableWritePlan(
  ...
  for (const variable of next) {
    ...
    if (!existing || existing.value !== variable.value) {
      upserts.push({ name, value: variable.value, platform_id: platformId });
    }
  }
