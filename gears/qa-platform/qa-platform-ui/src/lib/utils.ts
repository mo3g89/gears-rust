 ...
 */
/**
 * Render a platform option label as `name (version.build)`. Falls back to just
...
export function formatPlatformLabel(platform: {
  ...
  if (!name) return '';
  const version = platform?.version?.trim();
  if (!version) return name;
  const build = platform?.build?.trim();
 ...
 * or platform default branch is available — e.g. a multi-repo custom plan or
