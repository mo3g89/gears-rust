 ...
 * environment instead.** That is deliberately a behaviour change from legacy, and it is
 * the only reading that does something useful on this topology.
 *
 * # Two tiers: the explicit flag, then the sole environment
 *
 * 1. **`is_default`** — an operator marked one environment as the product's default. This
 *    is the answer whenever it exists, and it is the only thing that can disambiguate a
 *    product with several environments.
 * 2. **The product's only environment** — with exactly one candidate there is nothing to
 *    disambiguate, so requiring a flag would make the common single-environment setup fail
 *    for no reason.
 *
 * With several environments and **no** flag set this returns `null` rather than guessing.
 ...
 *
 * If more than one environment somehow carries the flag — the service clears the previous
 * holder, but that clear and the promotion are two statements with no transaction
 * around them — this still returns a single environment rather than `null`, because a UI
 ...
 *
 * An environment with no `product_id` is never a candidate: it is not attached to the
 * product whose plan is being launched.
 */
export function defaultEnvironmentForProduct(
  environments: EnvironmentInfo[],
  productId: string | null | undefined
): EnvironmentInfo | null {
  if (!productId) return null;
  const owned = environments.filter((environment) => environment.product_id === productId);
  const flagged = owned.find((environment) => environment.is_default);
 ...
 */
export function defaultEnvironmentLabel(resolved: EnvironmentInfo | null): string {
