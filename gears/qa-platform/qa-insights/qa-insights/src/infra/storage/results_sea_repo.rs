...
/// The dashboard's product filter, as a `repo_id IN (…)` predicate.
///
/// `Some(ids)` restricts to rows whose `repo_id` is one of `ids` — the set of
/// repositories the selected product owns, resolved by
/// `domain::service::dashboard::DashboardService::stats` from a plain
/// repository listing rather than from a checkout-derived test universe (see
/// that method's doc for why: a universe walk answers "no row for this
/// repository" indistinguishably for "not yet synced" and "genuinely has no
/// tests", and the first must not read as the second). `None` adds no
/// predicate at all, which is every row this tenant's scope already admits —
/// so an unscoped request pays no extra clause and behaves exactly as it did
/// before this parameter existed.
///
/// A row with a `NULL` `repo_id` matches no non-empty `ids` — not a decision
/// this function makes, since `is_in` never matches `NULL` in any dialect.
///
/// # Which rows that is, and it is **not** the same set `run_repo_id` excludes
///
/// `domain::service::ingest::plan_identity` — the function that stamps this
/// column at ingest time — writes `NULL` for **both** a custom-plan run and a
/// **collect** run: neither executes a plan, so neither has the single
/// `(repo_id, plan_path)` pair that function exists to give one. That is a
/// wider `NULL` set than the live-run half of this same filter admits:
/// `domain::service::dashboard::run_repo_id` reads a collect run's `repo_id`
/// directly off its target and **does** attribute it to a product. So a
/// collect run is scoped correctly wherever this endpoint reads live qa-runs
/// state — the two counts, both lists, the per-run trend — and is invisible
/// to every read this predicate guards, because a collect run's own rows
/// (case counts, not per-test results) never carried a `repo_id` to filter on
/// in the first place. The practical consequence is narrow: those reads
/// (`total_runs`, the daily trend, the 24-hour block, `flaky_tests`,
/// `quality_vectors_pass_rate`) were never going to reflect a collect run's
/// case counts anyway, unscoped or scoped, so this is not a new gap this
/// filter introduces — it is the existing one, restated so a reader does not
/// conclude the two halves agree when they do not.
fn repo_scope(repo_ids: Option<&[Uuid]>) -> Condition {
    repo_ids.map_or_else(Condition::all, |ids| {
        Condition::all().add(ResultColumn::RepoId.is_in(ids.iter().copied()))
    })
}
...
mod tests {
    ...
    async fn the_window_admits_finished_runs_at_or_after_since_and_no_others() {
        ...
        let counts = OrmResultsRepository
            .run_status_counts_since(&conn, &scope(tenant), since, None)
            .await
            .unwrap();
        assert_eq!(
    ...
    async fn the_counter_reads_are_scoped_to_the_caller() {
        ...
        let windowed = OrmResultsRepository
            .run_status_counts_since(&conn, &scope(mine), now(), None)
            .await
            .unwrap();
        assert_eq!(
            windowed
    ...
    async fn the_run_total_counts_runs_and_not_rows() {
        ...
        assert_eq!(
            OrmResultsRepository
                .count_ingested_runs(&conn, &scope(mine), None)
                .await
                .unwrap(),
            0,
        ...
        assert_eq!(
            OrmResultsRepository
                .count_ingested_runs(&conn, &scope(theirs), None)
                .await
                .unwrap(),
            1,
        );
    ...
    async fn the_kpi_window_counts_by_status_over_the_coalesced_instant() {
        ...
        assert_eq!(
            sorted_counts(
                OrmResultsRepository
                    .effective_status_counts(&conn, &scope(mine), from, None, None)
                    .await
                    .unwrap()
            ),
            vec![
        ...
        assert_eq!(
            sorted_counts(
                OrmResultsRepository
                    .effective_status_counts(&conn, &scope(mine), prev_from, Some(from), None)
                    .await
                    .unwrap()
            ),
        ...
        assert_eq!(
            sorted_counts(
                OrmResultsRepository
                    .effective_status_counts(&conn, &scope(theirs), from, None, None)
                    .await
                    .unwrap()
            ),
    ...
    async fn an_unfinished_runs_rows_are_windowed_by_the_runs_age_not_the_ingest_instant() {
        ...
        assert_eq!(
            sorted_counts(
                OrmResultsRepository
                    .effective_status_counts(&conn, &scope(tenant), from, None, None)
                    .await
                    .unwrap()
            ),
        ...
        assert_eq!(
            sorted_counts(
                OrmResultsRepository
                    .effective_status_counts(&conn, &scope(tenant), prev_from, Some(from), None)
                    .await
                    .unwrap()
            ),
        ...
        let failures = OrmResultsRepository
            .recent_failures(&conn, &scope(tenant), &["FAILED", "ERROR"], from, 10, None)
            .await
            .unwrap();
        assert_eq!(
    ...
    async fn recent_failures_are_the_newest_admissible_rows_up_to_the_limit() {
        ...
        let rows = OrmResultsRepository
            .recent_failures(&conn, &scope(mine), &["FAILED", "ERROR"], from, 2, None)
            .await
            .unwrap();
        ...
        let all = OrmResultsRepository
            .recent_failures(&conn, &scope(mine), &["FAILED", "ERROR"], from, 10, None)
            .await
            .unwrap();
        assert_eq!(
            all.len(),
        ...
        assert!(
            OrmResultsRepository
                .recent_failures(&conn, &scope(mine), &[], from, 10, None)
                .await
                .unwrap()
                .is_empty(),
    ...
    async fn the_flaky_window_is_seven_days_of_the_effective_timestamp_and_scoped() {
        ...
        let groups = OrmResultsRepository
            .flaky_groups(&conn, &scope(mine), &PASSED, &FAILED, since, 10, None)
            .await
            .unwrap();
    ...
    async fn a_flaky_read_with_an_empty_status_set_answers_empty() {
        ...
        assert_eq!(
            OrmResultsRepository
                .flaky_groups(&conn, &scope(tenant), &PASSED, &FAILED, since, 10, None)
                .await
                .unwrap()
                .len(),
            1,
        ...
        assert!(
            OrmResultsRepository
                .flaky_groups(&conn, &scope(tenant), &[], &FAILED, since, 10, None)
                .await
                .unwrap()
                .is_empty(),
        ...
        assert!(
            OrmResultsRepository
                .flaky_groups(&conn, &scope(tenant), &PASSED, &[], since, 10, None)
                .await
                .unwrap()
                .is_empty(),
    ...
    async fn the_aggregate_reads_are_accepted_by_real_postgres() {
        ...
        let windowed = OrmResultsRepository
            .run_status_counts_since(&conn, &scope(tenant), now() + Duration::minutes(30), None)
            .await
            .unwrap();
        assert_eq!(
            windowed
        ...
        assert_eq!(
            OrmResultsRepository
                .count_ingested_runs(&conn, &scope(tenant), None)
                .await
                .unwrap(),
            2,
            "two runs, four rows",
    ...
    async fn an_empty_status_set_answers_empty_rather_than_all_zeroes() {
        ...
        for (passed, failed) in [
            ...
            assert!(
                OrmResultsRepository
                    .file_status_counts(&conn, &scope(tenant), passed, failed, since, None)
                    .await
                    .unwrap()
                    .is_empty(),
            );
