//! Unit tests for qa-insights' GTS permission catalog.
//!
...
//!
//! `qa-insights_permission_instances` filters on
//! `INSTANCE_SUFFIX_PREFIX = "cf.qa.insights."`, which is what keeps this
//! gear's 15 instances separate from the 38 qa-catalog (25) and
//! qa-environments (13) register into the same process-wide
//! `toolkit_gts::inventory` -- their suffixes are `cf.qa.catalog.` and
//! `cf.qa.environments.` respectively, neither of which this prefix matches.

use toolkit_gts::{GTS_ID_PREFIX, InventoryInstance, gts_id};

const PERMISSION_TYPE_ID: &str = gts_id!("cf.toolkit.authz.permission.v1~");
const INSTANCE_SUFFIX_PREFIX: &str = "cf.qa.insights.";

/// Every qa-insights permission instance id -- one per enforced
...
const EXPECTED_PERMISSION_IDS: &[&str] = &[
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.jira_bug_create.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.jira_bug_list.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.jira_bug_update.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.jira_config_get.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.jira_config_update.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.notification_config_get.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.notification_config_test.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.notification_config_update.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.saved_view_create.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.saved_view_delete.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.saved_view_list.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.saved_view_update.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.test_result_collect.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.test_result_list.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.insights.test_result_rebuild.v1"),
];

fn qa_insights_permission_instances() -> Vec<&'static InventoryInstance> {
...
/// resource type's GTS id — `cf.qa.insights.saved_view.v1~` → `saved_view`.
///
/// Two strips, not one: `gts_id!` prepends `GTS_ID_PREFIX` (`"gts."`) at
/// compile time, so the constant holds `gts.cf.qa.insights.<entity>.v1~`. It is
...
fn derive_instance_id(resource_type: &str, action: &str) -> String {
    let pep_entity = resource_type
        ...
        .unwrap_or_else(|| {
            panic!("resource type {resource_type} is not a cf.qa.insights.<entity>.v1~ id")
        });
    format!("{PERMISSION_TYPE_ID}{INSTANCE_SUFFIX_PREFIX}{pep_entity}_{action}.v1")
}

#[test]
fn all_qa_insights_permissions_are_registered_in_inventory() {
    let entries = qa_insights_permission_instances();
    assert_eq!(
        entries.len(),
...
fn the_inventory_covers_every_expected_id() {
    let found: std::collections::BTreeSet<_> = qa_insights_permission_instances()
        .iter()
        .map(|e| e.instance_id)
        .collect();
    ...
    assert_eq!(
        found.len(),
        EXPECTED_PERMISSION_IDS.len(),
        "inventory contains qa-insights permission ids not in the expected set"
    );
}
...
fn expected_ids_match_the_derivation_rule() {
    let instances = qa_insights_permission_instances();
    for &expected in EXPECTED_PERMISSION_IDS {
        let entry = instances
            .iter()
...
fn the_catalog_matches_the_enforced_surface() {
    let cataloged: std::collections::BTreeSet<(String, String)> =
        qa_insights_permission_instances()
            .iter()
