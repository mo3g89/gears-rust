...
/// an out-of-range one is clamped rather than refused. `product_id` is likewise
/// optional and reaches the service unchanged — absent narrows nothing, and a
/// malformed one fails the extractor with a 400 before this handler runs.
#[tracing::instrument(
    skip(svc, ctx, query),
    fields(dashboard.days = ?query.days, dashboard.product_id = ?query.product_id)
)]
pub async fn dashboard(
    ...
    let stats = svc
        .dashboard
        .stats(&ctx, query.days, query.product_id)
        .await?;
