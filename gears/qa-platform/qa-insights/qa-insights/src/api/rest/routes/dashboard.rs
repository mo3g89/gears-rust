    ...
    let router = OperationBuilder::get("/qa/v1/dashboard")
        ...
        .description(
            "Run activity and the test counters behind it, in one payload. Live run state \
             ...
             deployment-wide. The attribution is through the run's own target: a run \
             naming a repository (directly, whether it is running a plan, a single test, \
             or enumerating a repository's cases) is attributed to that repository's \
             product; a run naming a custom plan spans repositories and is attributed to \
             none. unattributable_runs is the one field product_id does not narrow, and \
             it is not a measurement of the selected product: it counts the runs in the \
             same window that belong to no product at all, whichever product they would \
             otherwise have been under, so it is not the selected product's own dropped \
             runs - there is no way to identify those, which is what makes the runs \
             unattributable in the first place. It is 0 without product_id only because \
        ...
        .query_param_typed(
            "product_id",
            false,
            "Narrow every run-derived number to one product, through the run's \
             target. Absent narrows nothing; a value that is not a UUID is a 400.",
            "string",
        )
