    ...
    L: LicenseState,
{
    builder
        ...
        .query_param_typed(
            "group_by",
            false,
            "none, component, tag or environment. Defaults to none when absent; a present but \
...
fn register_overview(router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    ...
    let builder = OperationBuilder::get("/qa/v1/analytics/overview")
        ...
        .description(
            "Eight computed sections over one product's test universe, in one payload: a \
             ...
             universe so the chart remains a comparison. group_by=environment narrows nothing \
             ...
             a non-zero total from the static counts alone. The environment breakdown carries an \
    ...
    overview_query_params(builder)
        ...
        // qa-environments on the environment names. None of the three is laundered
    ...
    OperationBuilder::get("/qa/v1/analytics/plan/tests")
        ...
        .description(
            "Aggregated per-test analytics for one plan: the most recent execution's status, \
             environment, version, run and JIRA reference, plus pass/fail/total counts over every \
