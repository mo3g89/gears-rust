...
//! plan *id* that this architecture stores differently: a run id, a platform id
//! resolved through qa-environments, and a `(repo_id, plan_path)` pair.
//! [`AnalyticsListItemDto`] carries the argument for all three, and
//! [`PlatformGroupSummaryDto`] carries the one case where the *shape* of a
...
///
/// # One parameter, where legacy has three
///
...
/// every run-derived number). **Neither is expressible here yet**, and it is the
/// same gap `domain::service::dashboard::dashboard_run` records for
/// `product_key`: VHP-319 deleted legacy's product-version model, qa-catalog owns
/// products now, and `qa_runs_sdk::Run` carries nothing that identifies one. That
/// mapping is the plan's own largest open question, deferred to Task 20's Step 0,
/// so a `product_key` accepted here would have to be silently ignored — which is
...
pub struct DashboardRunDto {
    ...
    pub repo_id: Option<Uuid>,
    pub plan_path: Option<String>,
    /// The platform the run occupied, as an id. Legacy draws a platform *name*
...
pub struct FailedTestCardDto {
    ...
    pub repo_id: Option<Uuid>,
    pub plan_path: Option<String>,
    /// The platform the run occupied, as an id rather than legacy's name — the
...
pub struct AnalyticsListItemDto {
    ...
    /// See this type's header for why both this and [`Self::last_platform`] are
    /// present.
    pub last_platform_id: Option<Uuid>,
    /// `null` when no row named a platform **or** when qa-environments resolved
...
/// [`AnalyticsListItemDto`] with the platform name joined in.
///
...
fn list_item_dto(item: AnalyticsListItem, names: &HashMap<Uuid, String>) -> AnalyticsListItemDto {
    AnalyticsListItemDto {
        ...
        last_platform_id: item.last_platform_id,
        last_platform: item.last_platform_id.and_then(|id| names.get(&id).cloned()),
...
/// One bar of the platform breakdown.
///
...
#[derive(Debug, Clone)]
#[toolkit_macros::api_dto(response)]
pub struct PlatformGroupSummaryDto {
    pub platform_id: Uuid,
    /// `null` when qa-environments resolved no name for
    /// [`Self::platform_id`].
    pub platform: Option<String>,
    /// The size of the **whole** universe, not of the platform's own tests: each
...
pub struct GroupedSummariesDto {
    ...
    /// regression to hunt.
    pub platform: Vec<PlatformGroupSummaryDto>,
}
        ...
        Self {
            ...
            grouped: GroupedSummariesDto {
                ...
                tag: overview
                    ...
                    .map(GroupSummaryDto::from)
                    .collect(),
                platform: platform_bars(overview.grouped.platform, &names),
            },
        }
    }
}
...
///
/// See [`GroupedSummariesDto::platform`] for why the order moves and
/// [`PlatformGroupSummaryDto`] for why an unnamed bar is kept rather than
...
fn platform_bars(
...
) -> Vec<PlatformGroupSummaryDto> {
    let mut bars: Vec<PlatformGroupSummaryDto> = bars
        .into_iter()
        .map(|bar| PlatformGroupSummaryDto {
            platform_id: bar.platform_id,
            platform: names.get(&bar.platform_id).cloned(),
            total: bar.total,
            passed: bar.passed,
            failed: bar.failed,
            not_run: bar.not_run,
    ...
    bars.sort_by(|left, right| {
        let key = |bar: &PlatformGroupSummaryDto| {
            (
                u8::from(bar.platform.is_none()),
                bar.platform.clone().unwrap_or_default(),
                bar.platform_id,
            )
        };
...
/// `GroupBy` in legacy's wire spelling — `group_to_str`
/// (`analytics.rs:2095-2102`). Here for [`scope_to_str`]'s reason.
...
pub struct PlanTestAnalyticsDto {
    ...
    pub last_status: String,
    /// See this type's header for why this rides beside [`Self::last_platform`].
    pub last_platform_id: Option<Uuid>,
    /// `null` when no row named a platform **or** when qa-environments resolved
...
/// [`PlanTestAnalyticsDto`] with the platform name joined in — the same split
/// [`list_item_dto`] makes, over [`PlanTests::platform_names`].
...
/// `Vec<PlanTestAnalytics>` plus the resolved platform names, joined into one
/// array. Not a wrapper struct on the wire: legacy's own response is a bare
/// JSON array (`Json<Vec<TestAnalytics>>`), and the platform names have no
/// field of their own in it.
#[must_use]
...
mod tests {
    ...
    use qa_insights_sdk::{
        CoverageBuild, CoverageSummary, DailyStatusPoint, DashboardStats, FailedTestCard,
        FlakyTestCard, JiraPollerConfig, NotificationConfig, NotificationLogEntry,
    ...
    use super::{
        ...
        CoverageBuildDto, DailyStatusPointDto, DashboardStatsDto, FailedTestCardDto,
        FlakyTestCardDto, GroupBy, HashMap, JiraBug, JiraBugDto, JiraBugFilingDto, JiraConfig,
        JiraPollerConfigDto, JiraSettingsDto, NewSavedViewReq, NotificationConfigDto,
        NotificationLogEntryDto, NotificationPreviewDto, PlanBuildDistributionDto,
        PlanTestHistoryDto, RebuildOutcomeDto, RebuildReq, SavedViewDto,
    ...
    /// (`Option<String>`), and `platform_id`/`repo_id` (`Option<Uuid>`), with
    ...
    fn the_test_result_conversion_maps_each_field_to_its_own_name() {
        ...
        assert_eq!(json["platform_id"], Uuid::from_u128(3).to_string());
    ...
    fn a_failure_card_maps_every_column_to_its_own_key() {
        ...
        assert_eq!(json["platform_id"], "00000000-0000-0000-0000-0000000000c3");
    ...
    /// **The platform name is joined into both sections that carry an id**, and
    /// an id the map does not hold renders as `null` rather than as a UUID.
    ///
    ...
    #[test]
    fn the_platform_names_are_joined_into_the_bars_and_the_list_items() {
        ...
        assert_eq!(
            dto.lists.passed[0].last_platform.as_deref(),
            Some("Linux"),
            "a resolved id renders its name",
        );
        assert_eq!(
            dto.lists.passed[1].last_platform, None,
            "an unresolved id is null, not the UUID's string",
        );
        assert_eq!(dto.lists.passed[1].last_platform_id, Some(ghost));

        let bars: Vec<(Option<&str>, Uuid)> = dto
            .grouped
            .platform
            .iter()
            .map(|bar| (bar.platform.as_deref(), bar.platform_id))
            .collect();
        assert_eq!(
            bars,
    ...
    #[test]
    fn the_platform_bars_are_ordered_by_resolved_name_and_not_by_id() {
        ...
        let names: Vec<&str> = dto
            ...
            .platform
            .iter()
            .filter_map(|bar| bar.platform.as_deref())
            .collect();
    ...
    /// `u32` counters, and `repo_id` against `last_platform_id`. Each value
    ...
    fn the_list_item_conversion_maps_each_field_to_its_own_name() {
        ...
        assert_eq!(item.last_platform_id, Some(platform));
    ...
    /// platform id and the resolved name ride together, and an id absent from
    /// the resolved map renders `null` for the name — [`AnalyticsListItemDto`]'s
    /// same rule, applied here.
    #[test]
    fn plan_test_analytics_joins_the_resolved_platform_name() {
        ...
        assert_eq!(items[0].last_platform_id, Some(resolved));
        assert_eq!(items[0].last_platform.as_deref(), Some("Linux x86_64"));
        assert_eq!(
            (
                items[0].total_runs,
                items[0].pass_count,
    ...
    /// and `platform_id` in particular — the two fields
    ...
    fn a_jira_bug_row_maps_onto_its_dto_field_for_field() {
        ...
        assert_eq!(dto.platform_id, bug.platform_id);
