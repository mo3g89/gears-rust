...
//!
//! The dependency runs the other way: `api` is a transport over `domain`.
//! `domain::local_client` imported its error attribution from
//! `api::rest::error::as_jira_error`, which the plugin rework made more
//! visible by moving that client from `infra/` into `domain/`. A local client
//! is an in-process call that never touches HTTP; its error attribution must
//! not come from the HTTP layer. Review findings #15, #16.
//!
