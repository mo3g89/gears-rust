//! The qa-environments read that turns a platform id into the label a chart
//! draws, behind a port.
//!
...
//! (`manager/src/routes/analytics.rs:1085-1146`, `build_grouped_summaries`). This
//! architecture has no such column:
//! [`ExecRow::platform_id`](crate::domain::analytics::ExecRow::platform_id) is a
//! `Uuid`, because qa-environments owns the platform and a denormalized name
...
//! greppable: [`PlatformReader`] is the complete list of what qa-insights asks
...
//!
//! [`PlatformReader::names`] takes a **slice** of ids and answers with a map. It
...
//!
//! [`PlatformReader::default_branch`] is the per-platform branch override the
//! auto-rerun resolves **once** and reuses for both the plan lookup and the
...
//! read rather than a batch, unlike [`PlatformReader::names`]: the poller resolves one
//! bug's platform at a time, so there is no window of distinct ids to
...
pub trait PlatformReader: Send + Sync {
    /// The display name of each of `ids` that names a platform visible to `ctx`.
    ///
    ...
    /// `map.get(&id) == None` is the answer *"this id names no platform you can
    /// see"* — which happens for a platform deleted since the run executed, and
    /// for a platform in another tenant.
    ///
    ...
    /// `platform_id`'s default-branch **override**, or `None` when it has none
    /// (or does not resolve to a platform visible to `ctx`).
    ///
    /// Legacy `PlatformsService::get_platform_default_branch`
    /// (`manager/src/services/platforms.rs:846-848`), which is one column read
    /// on the platform row — no fallback chain of its own. `None` covers two
    /// legacy-distinct cases this port does not distinguish, deliberately, for
    /// [`Self::names`]'s own reason: a platform with the column unset, and a
    /// platform id naming nothing this caller can see (deleted, or another
    /// tenant's). Both mean the same thing to every caller of this method —
    /// resolve the run's branch some other way — and a caller must not be able
    /// to use this read to learn *which* of the two it got.
    ///
    /// # Errors
    ///
    /// [`DomainError::Internal`] for any transport or gateway failure, and
    /// [`DomainError::Forbidden`] when qa-environments refuses the subject.
    /// There is no not-found case: an unresolvable `platform_id` is `Ok(None)`,
    /// exactly like an id absent from [`Self::names`]' map.
