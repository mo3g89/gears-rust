...
/// # Errors
///
/// [`DomainError::Validation`] on `group_by`, carrying legacy's message verbatim.
pub fn parse_group(value: Option<&str>) -> Result<GroupBy, DomainError> {
    match value.unwrap_or("none").trim().to_ascii_lowercase().as_str() {
        ...
        "tag" => Ok(GroupBy::Tag),
        "platform" => Ok(GroupBy::Platform),
        _ => Err(DomainError::Validation {
            field: "group_by".to_owned(),
            message: "group_by must be one of: none, component, tag, platform".to_owned(),
        }),
    }
}
