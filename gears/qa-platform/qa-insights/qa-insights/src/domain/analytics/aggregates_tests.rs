...
//! * [`the_platform_grouping_does_not_narrow_the_universe`] — selecting a
//!   platform bar returns every test (`:1178`).
...
/// `GroupBy::Platform` falls to legacy's `_` arm together with `GroupBy::None`
/// (`:1178`) and narrows **nothing**.
///
/// The rows carry the platform and the universe does not, so there is nothing to
/// filter on — selecting a platform bar returns the whole universe rather than the
...
#[test]
fn the_platform_grouping_does_not_narrow_the_universe() {
    let universe = vec![
        grouped("tests/a.py", Some("cluster"), &["nightly"]),
        grouped("tests/b.py", Some("network"), &[]),
    ];

    for group_by in [GroupBy::Platform, GroupBy::None] {
        assert_eq!(
