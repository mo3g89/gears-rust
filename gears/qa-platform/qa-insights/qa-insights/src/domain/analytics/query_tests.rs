...
/// `analytics.rs:2115-2127`: an unknown `group_by` is refused with
/// `"group_by must be one of: none, component, tag, platform"`.
///
...
fn an_unknown_group_by_is_refused_and_a_blank_one_counts_as_unknown() {
    ...
    for bad in [Some(""), Some("   "), Some("plan"), Some("components")] {
        assert_eq!(
            refusal(parse_group(bad).expect_err("refused")),
            (
                "group_by".to_owned(),
                "group_by must be one of: none, component, tag, platform".to_owned()
            ),
            "for {bad:?}",
        );
    }
