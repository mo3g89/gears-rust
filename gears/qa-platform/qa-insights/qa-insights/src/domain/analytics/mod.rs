...
pub struct ExecRow {
    ...
    /// ([`PlatformReader`](crate::domain::ports::PlatformReader)) and Task 25b
    /// called it.** The analytics service resolves the distinct ids of a whole
    /// payload in one cross-gear read, and
    /// [`crate::api::rest::dto::PlatformGroupSummaryDto`] renders each bar with
...
pub struct UniverseFilter {
    ...
    /// `qa-runs/src/domain/service/runs.rs:1052-1055` ("this port records one
