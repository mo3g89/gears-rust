...
/// [`PlatformReader::names`](crate::domain::ports::PlatformReader::names), Task
...
///   [`PlatformReader`](crate::domain::ports::PlatformReader); what an id that
...
/// * **[`GroupBy::Platform`] narrows nothing at all.** It falls to legacy's `_`
///   arm together with [`GroupBy::None`] (`:1178`), so selecting a platform bar
///   returns the whole universe rather than the tests that ran there — the rows
///   carry the platform and the universe does not, so there is nothing to filter
///   on. Ported as-is under Phase B's standing instruction and pinned by
///   `the_platform_grouping_does_not_narrow_the_universe`, because an
...
pub fn apply_universe_group_filter(
    ...
    match group_by {
        ...
        GroupBy::Tag => universe
            ...
            .cloned()
            .collect(),
        // `GroupBy::None` and `GroupBy::Platform` both narrow nothing — see this
        // function's header for why the second one does not.
        GroupBy::None | GroupBy::Platform => universe.to_vec(),
    }
}
