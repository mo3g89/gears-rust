...
use crate::domain::ports::{
    CatalogReader, Clock, JiraClient, MailClient, PlatformReader, RunsLauncher, RunsReader,
    SlackClient,
};
...
pub(crate) struct ServiceDeps {
    ...
    /// The platform id → display name resolution, behind [`PlatformReader`].
    ...
    pub(crate) platforms: Arc<dyn PlatformReader>,
    ...
    N: NotifyRepository + Clone + 'static,
{
    ...
    pub(crate) fn new(
        ...
        let analytics = Arc::new(analytics::AnalyticsService::new(
            ...
            // holds the same `PlatformReader` for its default-branch
