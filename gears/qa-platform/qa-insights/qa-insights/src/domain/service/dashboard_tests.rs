...
///
/// `cpt-cf-qa-fr-insights-dashboard` (PRD §5.5, `cpt-cf-qa-fr-insights-dashboard`) requires "active **and
...
async fn the_listing_ceiling_is_two_hundred_and_the_service_asks_for_it() {
    ...
    fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
...
impl Fixture {
    ...
    /// One run's rows, on `repo_id`/`plan_path`, one `PASSED` and one `FAILED`
    /// of `test_name` — a group that is simultaneously a flaky group (both
    /// statuses present) and a recent failure (the `FAILED` row), finished
    /// **now** so it falls inside every window this endpoint reads.
    ///
    /// The one seed `a_product_filter_narrows_the_sql_aggregates_too` needs:
    /// every other fixture in this file hardcodes `repo_id` via
    /// [`Self::seed_named_at`], which cannot exercise a *product*-scoped read
    /// that must tell two repositories apart.
    async fn seed_for_repo(
        &self,
        tenant: Uuid,
        run_id: Uuid,
        repo_id: Uuid,
    ...
    /// [`Self::seed_for_repo`] with the statuses and the instant chosen.
    ///
    /// Both are needed to pin the *24-hour* pair of reads, which
    /// [`Self::seed_for_repo`]'s fixed shape cannot reach: `failed_prev_24h_count`
    /// and `pass_rate_prev_24h` come from a window that holds nothing when every
    /// row is stamped `now`, and a pass *rate* that is `1 passed / 1 failed` in
    /// both repositories is the same ratio scoped and unscoped, so it would agree
    /// with an unfiltered read by arithmetic rather than by narrowing.
    #[allow(
        clippy::too_many_arguments,
        reason = "every one is a dimension the product-filter test has to vary \
                  independently: two tenanted ids, the repository the filter \
                  narrows on, the file and plan the vector and flaky reads group \
                  by, the statuses that separate a pass rate from a count, and \
                  the instant that separates the two 24-hour windows"
    )]
    async fn seed_for_repo_at(
        &self,
        tenant: Uuid,
        run_id: Uuid,
        repo_id: Uuid,
        ...
        let files = statuses
            .iter()
            .map(|status| NewTestResult {
                test_file: format!("tests/{test_name}.py"),
...
/// A product filter narrows every run-derived number to that product's own
/// runs — the two counts, both lists and the per-run trend alike — and the
/// **other** product's run is absent from every one of them, not merely
/// subtracted from a smaller total.
///
/// # This is `list_repos`, not `list_universe` — the fix round 1 rewrite
///
/// The previous revision of this test attributed a run through
/// `CatalogReader::list_universe`, the same checkout-derived join
/// `domain::analytics::AnalyticsService` uses. That was wrong for a run
/// listing: a real qa-catalog review found it fails closed on an unsynced
/// repository (`an_unsynced_repository_still_counts_its_products_runs` below
/// pins the fix) and disagrees with the client about collect runs
/// (`a_collect_run_in_the_products_repository_is_included` below pins that
/// one). This test now attributes through `CatalogReader::list_repos` and
/// each run's own `repo_id`, matching `qa-platform-ui/src/lib/productScope.ts`
/// exactly.
#[tokio::test]
async fn a_product_filter_narrows_every_run_derived_number_and_excludes_the_other_products_run() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);
    let product_b = Uuid::from_u128(0xA2);
    let repo_a = Uuid::from_u128(0xC1);
    let repo_b = Uuid::from_u128(0xC2);

    fx.catalog.add_repo(test_repo(repo_a, product_a));
    ...
    let run_a = Run {
        ...
        ..run_in_state(RunState::Running, 0)
    };
    let included_run_id = run_a.id;
    fx.runs.add_run(run_a, vec![]);

    let run_b = Run {
        target: RunTarget::Plan {
            repo_id: repo_b,
            path: "plans/other.yaml".to_owned(),
        },
        ..run_in_state(RunState::Running, 5)
    };
    let excluded_run_id = run_b.id;
    fx.runs.add_run(run_b, vec![]);

    let unscoped = fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
    assert_eq!(
        unscoped.active_runs, 2,
        "both runs are active before any product filter is applied"
    );
    assert_eq!(unscoped.recent_runs.len(), 2);
    assert_eq!(
        fx.catalog.repo_reads(),
        0,
        "an unscoped request must not pay for a repository listing it does not need",
    );

    let scoped = fx
        .service
        .stats(&ctx(TENANT), None, Some(product_a))
        .await
        .unwrap();

    assert_eq!(scoped.active_runs, 1, "product_a admits only run_a");
    assert_eq!(scoped.queued_runs, 0);
    assert_eq!(
        scoped
            .recent_runs
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec![included_run_id],
        "run_b's own row must be absent, not merely subtracted from a smaller total",
    );
    assert!(
        scoped
            .active_runs_list
            .iter()
            .all(|run| run.run_id != excluded_run_id),
        "run_b must not appear in the active list either",
    );
    assert_eq!(scoped.active_runs_list.len(), 1);
    assert_eq!(
        scoped
            .recent_run_test_trend
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec![included_run_id],
        "the trend chart's x-axis must narrow with the run list it is derived from",
    );
    assert_eq!(fx.catalog.repo_reads(), 1);
    assert!(
        fx.catalog.requests().is_empty(),
        "attributing a run to a product must not consult list_universe at all",
    );
}

/// **C1's fix, pinned.** A repository that has never synced —
/// [`test_repo`]'s own `last_synced_at: None` — still counts its product's
/// runs. `list_universe` would answer empty for such a repository (nothing
/// registered via [`FakeCatalog::add`]), and if the product filter still
/// depended on that read even indirectly, this run would silently vanish —
/// "Active: 0" on a product whose run is plainly running. It must not: qa-runs
/// is the authority for whether a run exists, and qa-catalog's sync state is
/// unrelated to that question.
#[tokio::test]
async fn an_unsynced_repository_still_counts_its_products_runs() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);
    let repo_a = Uuid::from_u128(0xC1);
    fx.catalog.add_repo(test_repo(repo_a, product_a));
    ...
    assert_eq!(
        scoped.active_runs, 1,
        "an unsynced repository must not fail the product filter closed",
    );
    assert!(
        fx.catalog.requests().is_empty(),
        "list_universe (the checkout-derived read) must never be consulted \
         to decide whether a run belongs to a product",
    );
}

/// **C2's fix, pinned.** A collect run in the product's own repository is
/// included — `RunTarget::Collect` carries a `repo_id` exactly as `Plan` and
/// `Test` do, and the client attributes it the same way
/// (`qa-platform-ui/src/lib/productScope.ts`). Before this round's fix,
/// `run_repo_id` used `plan_identity`, which maps every `Collect` target to
/// `(None, None)` — correct for ingest's row projection, wrong here, and this
/// test is what would have caught it.
#[tokio::test]
async fn a_collect_run_in_the_products_repository_is_included() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);
    let repo_a = Uuid::from_u128(0xC1);
    fx.catalog.add_repo(test_repo(repo_a, product_a));
    ...
    assert_eq!(
        scoped.active_runs, 1,
        "a collect run in the product's own repository must count",
    );
    assert_eq!(
        scoped
            .recent_runs
...
/// A custom-plan run matches no product — a disclosed gap, not a silent one.
/// [`run_repo_id`] answers `None` for [`RunTarget::CustomPlan`] because a
/// custom plan spans repositories and this gear has no port to resolve the
/// expansion `qa-platform-ui/src/lib/productScope.ts`'s
/// `productIdOfCustomPlan` performs client-side. Pinned so that closing this
/// gap in a later round is a deliberate change to a named test rather than an
/// unnoticed behaviour change.
///
/// Also pins `unattributable_runs`, added for `ProductScopeNotice`: `1` on the
/// scoped request that dropped this run, `0` on the unscoped one that kept
/// it — the field must not count every custom-plan run in the deployment,
/// only the ones a filter *this* request actually applied could not place.
#[tokio::test]
async fn a_custom_plan_run_matches_no_product() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);

    let run = Run {
        target: RunTarget::CustomPlan { id: Uuid::new_v4() },
        ..run_in_state(RunState::Running, 0)
    };
    fx.runs.add_run(run, vec![]);
    ...
    assert_eq!(
        scoped.active_runs, 0,
        "a custom-plan run is excluded from every product-scoped dashboard",
    );
    assert_eq!(
        scoped.unattributable_runs, 1,
        "the dropped custom-plan run must be counted for the notice",
    );

    let unscoped = fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
    assert_eq!(
        unscoped.active_runs, 1,
        "the same run is present on the deployment-wide dashboard",
    );
    assert_eq!(
        unscoped.unattributable_runs, 0,
        "an unscoped request drops nothing, so this must not count the run \
         just because it happens to be unattributable",
    );
}

/// A product filter that excludes a run belonging to a **different** product
/// must not inflate `unattributable_runs` — that run has a perfectly good
/// attribution, just not to the product asked for, which is the filter
/// working rather than the gap this field exists to surface.
#[tokio::test]
async fn a_different_products_run_does_not_count_as_unattributable() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);
    let product_b = Uuid::from_u128(0xA2);
    let repo_b = Uuid::from_u128(0xC2);
    ...
    assert_eq!(
        scoped.active_runs, 0,
        "product_b's run is correctly excluded"
    );
    assert_eq!(
        scoped.unattributable_runs, 0,
        "excluded for belonging to another product, not for being unattributable",
    );
}

/// **C3's fix, pinned — all six repo-scoped reads, not four.** The
/// `qa_test_results` reads that have no run-id parameter to narrow by
/// (`count_ingested_runs`, `run_status_counts_since`, `recent_failures`,
/// `flaky_groups`, both `effective_status_counts` windows and
/// `file_status_counts`) narrow by `repo_id` exactly as the live-run half does,
/// closing the mixed-scope page a review found: three of nine rendered surfaces
/// scoped and six silently global, with no marker distinguishing them.
///
/// **This doc said "five" and asserted four.** The final review verified the
/// gap by mutation: replacing `repo_ids` with `None` at both
/// `effective_status_counts` call sites and at `file_status_counts` left all 52
/// dashboard tests passing, so `failed_24h_count`, `failed_prev_24h_count`,
/// `pass_rate_24h`, `pass_rate_prev_24h` and `quality_vectors_pass_rate` —
/// three KPI tiles and the whole Quality Vectors card — were threaded and
/// pinned by nothing. "A parameter accepted and ignored" is the failure the
/// plan named as the thing to guard against, and it had shipped in three
/// places. The three groups of assertions at the end of this test are what
/// catches it.
///
/// # The fixture, and why each part of it is shaped the way it is
///
/// Two repositories in two products. Each has a run stamped **now** — inside
/// every one of the four windows this endpoint reads — and a second run stamped
/// **36 hours ago**, which is inside the *previous* 24-hour window and outside
/// the current one, since nothing else in this file can reach the `prev` pair
/// at all.
///
/// The now-runs carry both a `PASSED` and a `FAILED` row of one test, which is
/// simultaneously a flaky group (needs both statuses) and a recent failure
/// (needs one). `repo_b`'s carries a **second** `FAILED`: with one of each in
/// both repositories the pass *rate* is `0.5` scoped and unscoped, so it would
/// agree with an unnarrowed read by arithmetic rather than by narrowing. The
/// two 36-hour rows are `PASSED` in `repo_a` and `FAILED` in `repo_b` for the
/// same reason, in the other direction.
///
/// The universe entries give `repo_a`'s file the `Security` vector and
/// `repo_b`'s the `Resilience` one, so an unnarrowed `file_status_counts`
/// renders a vector that belongs to the other product — visible as an extra
/// array entry rather than as a larger number, which is the harder mutation to
/// miss. The 36-hour files have no universe entry and so contribute no vector.
#[tokio::test]
async fn a_product_filter_narrows_the_sql_aggregates_too() {
    let fx = Fixture::new().await;
    let product_a = Uuid::from_u128(0xA1);
    let product_b = Uuid::from_u128(0xA2);
    let repo_a = Uuid::from_u128(0xC1);
    ...
    let run_b = Uuid::from_u128(0xD2);
    fx.seed_for_repo(TENANT, run_a, repo_a, "flaky_a", "plans/a.yaml")
        .await;
    fx.seed_for_repo_at(
        TENANT,
        run_b,
    ...
    // 36 hours back: inside the *previous* 24-hour window and outside the
    // current one. Single-status on purpose — neither forms a flaky group.
    let thirty_six_hours_ago = OffsetDateTime::now_utc() - Duration::hours(36);
    fx.seed_for_repo_at(
        TENANT,
        Uuid::from_u128(0xD3),
        repo_a,
    ...
    let unscoped = fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
    assert_eq!(unscoped.total_runs, 4, "all four ingested runs count");
    assert_eq!(
        unscoped.flaky_tests.len(),
        2,
        "both products' flaky groups are present"
    );
    assert_eq!(
        trend_totals(&unscoped),
        (3, 4),
        "every row of both products is in the daily trend"
    );
    assert_eq!(
        (unscoped.failed_24h_count, unscoped.pass_rate_24h),
        (3, Some(2.0 / 5.0)),
        "the current 24-hour window holds both products' now-rows",
    );
    assert_eq!(
        (unscoped.failed_prev_24h_count, unscoped.pass_rate_prev_24h),
        (1, Some(1.0 / 2.0)),
        "the previous window holds repo_a's PASSED and repo_b's FAILED",
    );
    assert_eq!(
        unscoped
            .quality_vectors_pass_rate
            .iter()
            .map(|item| (item.vector.as_str(), item.passed, item.failed, item.total))
            .collect::<Vec<_>>(),
        vec![("Resilience", 1, 2, 3), ("Security", 1, 1, 2)],
        "both products' vectors, ranked by total descending",
    );
    ...
    assert_eq!(
        scoped.total_runs, 2,
        "only repo_a's two runs are ingested here"
    );
    assert_eq!(
        scoped
            .flaky_tests
            .iter()
            .map(|card| card.repo_id)
            .collect::<Vec<_>>(),
        vec![Some(repo_a)],
        "repo_b's flaky group must be absent, not merely subtracted from a smaller total",
    );
    assert!(
        scoped
            .failed_recent
            .iter()
            .all(|card| card.repo_id == Some(repo_a)),
        "every recent failure must belong to the selected product's repository",
    );
    assert_eq!(
        trend_totals(&scoped),
        (2, 1),
        "the daily trend narrows to repo_a's two passed and one failed row",
    );
    // `effective_status_counts`, current window. Both the counter and the ratio:
    // `repo_b` contributes two failures, so an unnarrowed read reports `3` and
    // `0.4` here.
    assert_eq!(
        (scoped.failed_24h_count, scoped.pass_rate_24h),
        (1, Some(1.0 / 2.0)),
        "the 24-hour KPI pair narrows to repo_a's own rows",
    );
    // `effective_status_counts`, previous window — the second of the two call
    // sites, and the one no other test in this file reaches. Unnarrowed it
    // reports `1` and `0.5`; narrowed, repo_a's window holds one `PASSED` and
    // nothing else.
    assert_eq!(
        (scoped.failed_prev_24h_count, scoped.pass_rate_prev_24h),
        (0, Some(1.0)),
        "the previous 24-hour KPI pair narrows too",
    );
    // `file_status_counts`. Asserted as the whole array: `Resilience` belongs to
    // repo_b's file, so an unnarrowed read leaves a second entry here — a
    // foreign product's vector rendered on this product's card.
    assert_eq!(
        scoped
            .quality_vectors_pass_rate
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec![("Security", 1, 1, 2)],
        "repo_b's vector must be absent, not merely smaller",
    );
}

/// **M1: `product_id` adds a grant precondition, and it is the caller's whole
/// dashboard that fails without it.**
///
/// `list_repos` is called unconditionally once a `product_id` is given —
/// deliberately, since a product filter that silently narrowed nothing when
/// qa-catalog refused would be worse than a 403 — so a caller holding
/// `qa.test_result/list` but not `qa.test_repo/list` now gets `403` on the
/// *entire* payload rather than a dashboard missing one section. The endpoint
/// description states it; nothing pinned it.
///
/// The two halves have to be asserted together, because the property is the
/// **difference**: the same caller, the same refusing catalog, `200` unscoped
/// and `403` scoped. Asserting only the 403 would pass for an implementation
/// that needed the grant either way.
///
/// The fixture ingests nothing on purpose. `stats` skips `list_universe`
/// entirely when no file had a row in the window (legacy's own short-circuit),
/// so the only catalog read either call can make is the product filter's own —
/// which is what makes the unscoped `200` mean "no catalog read at all" rather
/// than "the universe read happened to be tolerated".
#[tokio::test]
async fn a_product_scoped_request_needs_a_catalog_grant_an_unscoped_one_does_not() {
    let fx = Fixture::new().await;
    fx.catalog.fail_reads(CatalogFailure::Forbidden);

    assert!(
        fx.service.stats(&ctx(TENANT), None, None).await.is_ok(),
        "an unscoped dashboard reads no repository listing, so a refusing catalog cannot fail it",
    );
    assert_eq!(
        fx.catalog.repo_reads(),
        0,
        "and it must not have asked, rather than asked and ignored the refusal",
    );

    assert!(
        matches!(
            fx.service
                .stats(&ctx(TENANT), None, Some(Uuid::from_u128(0xA1)))
                .await,
            Err(DomainError::Forbidden)
        ),
        "the same caller, scoped to a product, is refused the whole payload",
    );
    assert_eq!(
        fx.catalog.repo_reads(),
        1,
        "the refusal came from the product filter's own read",
    );
}

/// **M2: a product that owns no repository narrows to nothing, not to
/// everything.**
///
/// The filter is `Option<Vec<Uuid>>`, and the two "empty" cases are opposites:
/// `None` is "no product asked for, narrow nothing" and `Some(vec![])` is "this
/// product owns no repository, so nothing can match". They are one `is_some()`
/// apart in `stats`, and the SQL half is not obvious either — `SeaQuery` renders
/// an empty `IN ()` as `1 = 2` rather than as a syntax error or a no-op
/// (`infra::storage::results_sea_repo`'s `status_count` records the same
/// special-case), so the aggregates come back zero. Correct, and untested: a
/// flattening of `Some(vec![])` to `None` anywhere on that path would answer
/// with the **whole deployment's** numbers under a product that owns nothing,
/// which is the loudest possible version of this endpoint's failure mode.
///
/// A run and its rows exist, under another product's repository, so "zero" here
/// means "narrowed away" rather than "nothing was seeded".
#[tokio::test]
async fn a_product_owning_no_repository_narrows_every_number_to_zero() {
    let fx = Fixture::new().await;
    let product_without_repos = Uuid::from_u128(0xA1);
    let product_b = Uuid::from_u128(0xA2);
    let repo_b = Uuid::from_u128(0xC2);
    fx.catalog.add_repo(test_repo(repo_b, product_b));
    fx.catalog.add(
    ...
    let unscoped = fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
    assert_eq!(
        (unscoped.total_runs, unscoped.active_runs),
        (1, 1),
        "the fixture really is non-empty, so the zeros below are the filter's doing",
    );

    let scoped = fx
        .service
        .stats(&ctx(TENANT), None, Some(product_without_repos))
        .await
        .unwrap();
    assert_eq!(scoped.total_runs, 0, "no repository, no ingested run");
    assert_eq!(scoped.active_runs, 0, "and no live run either");
    assert_eq!(trend_totals(&scoped), (0, 0), "an empty daily trend");
    assert_eq!(
        (scoped.failed_24h_count, scoped.pass_rate_24h),
        (0, None),
        "an empty KPI window is no data rather than 0%",
    );
    assert!(scoped.flaky_tests.is_empty(), "no flaky group");
    assert!(scoped.failed_recent.is_empty(), "no recent failure");
    assert!(
        scoped.quality_vectors_pass_rate.is_empty(),
        "and no quality vector, which is the read that joins the catalog",
    );
    assert_eq!(
        scoped.unattributable_runs, 0,
        "the run was excluded for belonging to another product, not for being unattributable",
    );
}
...
async fn a_denied_caller_reads_nothing_and_does_not_reach_qa_runs() {
    ...
    assert!(matches!(
        fx.service.stats(&ctx(TENANT), None, None).await,
        Err(DomainError::Forbidden)
    ));
    assert_eq!(
        fx.runs.listings(),
...
async fn the_dashboard_reports_the_seven_day_flaky_tests() {
    ...
    let stats = fx.service.stats(&ctx(TENANT), None, None).await.unwrap();
...
async fn the_callers_day_count_is_clamped_and_governs_the_trend_length() {
    ...
    for (asked, expected) in [(None, 14), (Some(1), 3), (Some(365), 90), (Some(30), 30)] {
        let stats = fx.service.stats(&ctx(TENANT), asked, None).await.unwrap();
        assert_eq!(
