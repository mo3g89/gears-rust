...
//! [`PlatformReader`](crate::domain::ports::PlatformReader), which Task 25a added
...
use crate::domain::ports::{CatalogReader, Clock, PlatformReader, RunsReader};
...
/// An in-memory qa-environments platform table, behind [`PlatformReader`].
///
...
/// contract (`PlatformReader::names`, "an id that resolves to nothing is absent
...
/// [`PlatformReader`]'s signature is shaped for and nothing else can see: the
...
pub struct FakePlatforms {
    ...
    /// Every `ids` slice [`PlatformReader::names`] was called with, in order.
    batches: Mutex<Vec<Vec<Uuid>>>,
    /// Makes every [`PlatformReader::names`] fail with
    ...
    /// Per-platform default-branch overrides — [`PlatformReader::default_branch`],
...
#[async_trait]
impl PlatformReader for FakePlatforms {
    async fn names(
        &self,
        _ctx: &SecurityContext,
        ids: &[Uuid],
