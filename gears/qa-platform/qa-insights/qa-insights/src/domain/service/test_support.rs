...
pub struct FakeCatalog {
    ...
    /// Registered repositories, behind [`CatalogReader::list_repos`] — the
    /// dashboard's product filter reads this, never [`Self::entries`], because
    /// the whole point of that read is that it does **not** depend on whether a
    /// repository has ever synced (unlike `list_universe`, which only ever sees
    /// what [`Self::add`] registered).
    repos: Mutex<Vec<TestRepository>>,
    /// How many times [`CatalogReader::list_repos`] was called. The one thing
    /// [`Self::requests`] cannot see, since that field is `list_universe`'s own.
...
impl FakeCatalog {
    /// Register one repository, behind [`CatalogReader::list_repos`].
    ///
    /// Independent of [`Self::add`]: a repository can be registered here with
    /// no [`UniverseTest`] entry at all, which is the fixture for "registered
    /// but never synced" — [`test_repo`]'s own `last_synced_at: None` is that
    /// state, and `CatalogReader::list_repos`'s doc is the reason it matters
    /// that this still returns the repository rather than omitting it.
    pub fn add_repo(&self, repo: TestRepository) {
        self.repos.lock().unwrap().push(repo);
    }

    /// How many times [`CatalogReader::list_repos`] was called.
    #[must_use]
    pub fn repo_reads(&self) -> usize {
        self.repo_reads.load(Ordering::SeqCst)
    }
...
impl CatalogReader for FakeCatalog {
    async fn list_repos(&self, _ctx: &SecurityContext) -> Result<Vec<TestRepository>, DomainError> {
        self.repo_reads.fetch_add(1, Ordering::SeqCst);
...
///
/// One repository, owned by `product_id`, behind [`FakeCatalog::add_repo`].
///
/// `last_synced_at: None` and `sync_error: None` deliberately: the dashboard's
/// product filter must attribute a run to its repository's product whether or
/// not that repository has ever synced, and a fixture that always stamped a
/// sync instant would never exercise that.
#[must_use]
pub fn test_repo(id: Uuid, product_id: Uuid) -> TestRepository {
    TestRepository {
        id,
        product_id,
        name: "repo".to_owned(),
...
impl CatalogReader for SlowCatalog {
    async fn list_repos(&self, ctx: &SecurityContext) -> Result<Vec<TestRepository>, DomainError> {
        ...
        self.inner.list_repos(ctx).await
    }
