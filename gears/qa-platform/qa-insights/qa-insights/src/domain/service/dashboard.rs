...
//! that it is.** `docs/PRD.md:575-581` requires three things: this dashboard
...
//!   [`crate::domain::ports::PlatformReader`] is not that port** and does not
    ...
    R: ResultsRepository,
{
    ...
    ///
    /// **It is not deterministic from the caller's grants alone.** The universe
    /// read is skipped entirely when no test file had **any** row in the window
    /// (legacy's own short-circuit — see [`Self::stats`]' body), so the *same*
    /// caller on the *same* deployment gets `200` while the last seven days hold
    /// no result row and `403` once one lands. One `SKIPPED` row is enough to flip
    /// it, because the read groups on the file rather than on the status. A
    /// deployment diagnosing an intermittent 403 here should look at the grant,
    /// not at qa-catalog's health.
    ///
    ...
    pub async fn stats(
        ...
        let runs = self.runs.list_recent_runs(ctx, RUN_PAGE).await?;
        ...
        let windowed = self
            .results
            .run_status_counts_since(&conn, &scope, since)
            .await?;
        let total_runs = self.results.count_ingested_runs(&conn, &scope).await?;
        ...
        let current = self
            .results
            .effective_status_counts(&conn, &scope, kpi_from, None)
            .await?;
        let previous = self
            .results
            .effective_status_counts(&conn, &scope, prev_from, Some(kpi_from))
            .await?;
        let failures = self
            .results
            .recent_failures(
        ...
        // this needs the same partition per group.
...
///   (`docs/PRD.md:579` — one clause of a requirement this endpoint does not
