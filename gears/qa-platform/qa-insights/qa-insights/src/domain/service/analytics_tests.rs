...
/// and ordered — the shape `PlatformReader::names` is signed for.
#[test]
...
/// `PlatformReader::names`' slice signature exists for and that
...
/// to an unlabelled chart. `PlatformReader::names`' own `# Errors` says the port
...
impl Fixture {
    async fn with_authz(authz: Arc<dyn AuthZResolverClient>) -> Self {
        ...
        let service = AnalyticsService::new(
            ...
            Arc::clone(&platforms) as Arc<dyn crate::domain::ports::PlatformReader>,
