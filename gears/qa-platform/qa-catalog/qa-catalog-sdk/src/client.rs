...
use crate::models::{
    BundleRequest, CustomPlan, NewCustomPlan, NewTestRepository, Plan, Product, SshKey,
    SyncRequest, TestBundle, TestFileMeta, TestRepository, TestRepositoryUpdate, UniverseTest,
};
...
pub trait QaCatalogClientV1: Send + Sync {
    ...
    /// `folder` — as a full replace (`folder: None` moves it back to the
    /// root).
