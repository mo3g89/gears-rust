//! Tests for the per-sync ssh-agent.
//!
...
use super::{
    HOST_KEY_VERIFICATION_OPTIONS, SshAgent, is_encrypted_private_key, openssh_key_cipher,
    ssh_command_for,
};
...
/// when `QA_CATALOG_REQUIRE_SSH_TOOLS=1` is set, which CI is expected to
...
fn require_openssh() -> bool {
    ...
    if !available {
        assert!(
            ...
            "openssh client tools are missing but QA_CATALOG_REQUIRE_SSH_TOOLS is set: \
             these tests prove the key never reaches disk and MUST NOT be skipped here"
        );
        eprintln!(
            "skipping: openssh client tools not available (set \
             QA_CATALOG_REQUIRE_SSH_TOOLS=1 to make this a failure)"
        );
    }
    available
}
...
fn ssh_command_carries_agent_socket_batchmode_and_host_key_options() {
    ...
    assert!(
        ...
        "must point at the per-sync agent socket: {cmd}"
    );
...
fn the_agent_socket_path_is_quoted_against_word_splitting() {
    let cmd = ssh_command_for(
        Path::new("/var/folders/my tmp/qa-catalog-ssh-1/agent.sock"),
        Path::new("/var/folders/my tmp/qa-catalog-ssh-1/id.pub"),
    );
    assert!(
        cmd.contains("-o IdentityAgent='/var/folders/my tmp/qa-catalog-ssh-1/agent.sock'"),
...
fn passphrase_protected_keys_are_detected_and_named() {
    ...
    let Err(err) = SshAgent::start_with_key(&encrypted) else {
...
fn agent_socket_and_directory_are_removed_on_drop() {
    ...
    let (socket, dir) = {
        let agent = SshAgent::start_with_key_in(root.path(), &pem).unwrap();
...
fn a_failed_key_load_leaves_no_agent_directory_behind() {
    ...
    let Err(err) = SshAgent::start_with_key_in(root.path(), "this is not a private key at all")
    else {
        panic!("malformed key material must be refused");
    };
    assert!(matches!(
        err,
        crate::domain::error::DomainError::SyncFailed { .. }
    ));
...
fn a_refused_passphrase_key_creates_no_agent_directory() {
    ...
    let classic = "-----BEGIN RSA PRIVATE KEY-----\n\
                   ...
                   AAAA\n\
                   -----END RSA PRIVATE KEY-----\n";
    let Err(_) = SshAgent::start_with_key_in(root.path(), classic) else {
        panic!("a passphrase-protected key must be refused");
    };
    assert_eq!(
...
fn private_key_never_touches_the_filesystem() {
    ...
    let agent = SshAgent::start_with_key(&pem).unwrap();
