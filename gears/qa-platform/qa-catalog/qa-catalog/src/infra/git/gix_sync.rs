...
//! see [`super::ssh_agent`] for the mechanism and for the deliberate
//! host-key-verification weakening. An SSH remote with no `credential_ref`
...
impl SshSetup {
    ...
    fn prepare(url: &str, credential: Option<&str>) -> Result<Self, DomainError> {
        ...
        Ok(Self {
            agent: Some(SshAgent::start_with_key(pem)?),
        })
    }
...
mod ssh_invocation_tests {
    ...
    fn gix_actually_invokes_the_configured_ssh_command_for_an_ssh_remote() {
        ...
        // single-quoted, exactly as `ssh_agent::ssh_command_for` builds it:
