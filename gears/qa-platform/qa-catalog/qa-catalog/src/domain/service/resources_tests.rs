...
fn every_resource_descriptor_carries_its_own_name_const() {
    assert_eq!(
        resources::TEST_REPO.name(),
        resources::TEST_REPO_NAME,
        "resources::TEST_REPO was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::PLAN.name(),
        resources::PLAN_NAME,
        "resources::PLAN was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::CUSTOM_PLAN.name(),
        resources::CUSTOM_PLAN_NAME,
        "resources::CUSTOM_PLAN was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::PRODUCT.name(),
        resources::PRODUCT_NAME,
        "resources::PRODUCT was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::SSH_KEY.name(),
        resources::SSH_KEY_NAME,
        "resources::SSH_KEY was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::BUNDLE.name(),
        resources::BUNDLE_NAME,
        "resources::BUNDLE was built from the wrong *_NAME const",
    );
}
