...
//! crate has no offender and never had one; the gate is here so that it cannot
//! acquire one quietly. Its three sibling gears each did — two local clients
//! taking their error attribution from `api::rest::error`, a service taking a
//! byte cap from `api::rest::sse`, and a domain test reaching for REST DTOs —
//! and a gate installed only where the offenders were leaves the rest
//! unguarded. Review findings #15, #16, #39.
//!
