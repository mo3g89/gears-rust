...
///
/// Seeded **unbound** (`plugin_instance_id: None`), which is what these
/// fixtures need: none of them resolves a plugin, and a fixture carrying a
/// plugin id would be asserting a binding no test here checks. The registry's
/// own tenant-scoping test binds its product explicitly.
...
/// A **second** registered product plugin, so a rebind has somewhere to go.
///
...
/// The minimum [`QaProductPluginV1`] a fixture needs: enough to be *present*
/// in the `ClientHub`, which since Task 20 is what `create_product` requires.
///
/// It declares one non-secret field and observes nothing. Tests that care what
/// a plugin *says* build their own double — `plugin_registry_tests`'
/// `MarkerPlugin` for the catalogue, and qa-environments' `ScriptedPlugin` for
/// observation. This one exists so a product can be created at all.
...
impl QaProductPluginV1 for FixturePlugin {
    ...
    ) -> Result<Vec<qa_product_sdk::plugin::CredentialClassification>, PluginFailure> {
        Ok(Vec::new())
    }
    ...
    async fn observe(
        ...
        unimplemented!("qa-catalog fixtures never observe")
    }

    async fn prepare_run_access(
        &self,
        _env: &qa_product_sdk::plugin::EnvironmentHandle<'_>,
    ) -> Result<qa_product_sdk::access::RunAccess, PluginFailure> {
        unimplemented!("qa-catalog fixtures never dispatch")
    }

    fn runner(
        &self,
        _observed: Option<&qa_product_sdk::observation::ObservedAttrs>,
    ) -> qa_product_sdk::access::RunnerSpec {
        qa_product_sdk::access::RunnerSpec::default()
    }
...
/// A [`QaProductRegistry`] whose hub holds [`FIXTURE_PLUGIN_INSTANCE_ID`]
/// and [`FIXTURE_PLUGIN_INSTANCE_ID_B`].
///
/// # Why the hub is no longer empty
///
...
///
/// Registering the one id the fixtures use is better than giving the service a
/// permissive presence double: it exercises the real `ClientHub` lookup, so
/// these fixtures would catch a mismatch between the scope the service asks
/// under and the scope a plugin registers with.
///
/// `list_registered_plugins` still finds no types-registry client here, which
/// remains the correct answer for a fixture that registered none — the
...
fn fixture_plugin_registry(
    ...
    for instance_id in [FIXTURE_PLUGIN_INSTANCE_ID, FIXTURE_PLUGIN_INSTANCE_ID_B] {
        hub.register_scoped::<dyn QaProductPluginV1>(
            ...
            Arc::new(FixturePlugin) as Arc<dyn QaProductPluginV1>,
        );
    }
    Arc::new(QaProductRegistry::new(
        db,
...
/// A [`QaProductRegistry`] over the REAL `OrmProductsRepository` and the
/// [`TenantScopedAuthZ`] double, resolving plugins out of `client_hub`.
///
/// Separate from [`build_services`] because the registry deliberately does
/// not live in the DI container (see `gear::init`). Pass the same `Db` those
/// services were built with to have both see the same rows.
pub fn build_plugin_registry_tenant_scoped(
    db: Db,
    client_hub: Arc<ClientHub>,
) -> QaProductRegistry<OrmProductsRepository> {
...
pub fn build_services_tenant_scoped_with_credstore(
    ...
    let plugin_registry = fixture_plugin_registry(Arc::clone(&db), Arc::new(TenantScopedAuthZ));
