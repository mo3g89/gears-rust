...
impl QueueRepository for OrmQueueRepository {
    async fn insert<C: DBRunner>(
        ...
        let am = QueueAM {
            ...
            platform_id: ActiveValue::Set(row.platform_id),
    ...
    async fn queued_depth<C: DBRunner>(
        ...
        let count = QueueEntity::find()
            .filter(
                Condition::all()
                    .add(Expr::col(QueueColumn::PlatformId).eq(platform_id))
                    .add(queued()),
            )
            .secure()
            .scope_with(scope)
    ...
    async fn platforms_with_queued_rows<C: DBRunner>(
        ...
        let rows: Vec<QueuedPlatformRow> = QueueEntity::find()
            ...
            .project_all(runner, |query| {
                query
                    .select_only()
                    .column(QueueColumn::PlatformId)
                    .column(QueueColumn::TenantId)
                    .group_by(QueueColumn::PlatformId)
    ...
    async fn list_for_read<C: DBRunner>(
        ...
        let filter = match platform_id {
            Some(platform_id) => {
                Condition::all().add(Expr::col(QueueColumn::PlatformId).eq(platform_id))
            }
            None => Condition::all(),
        };
