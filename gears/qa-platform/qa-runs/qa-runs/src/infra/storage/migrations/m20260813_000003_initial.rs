...
//!    qa-environments client. `qa_platform_leases` is keyed on a bare
//!    `platform_id`, so the lease is not tenant-partitioned. See the comment on
//!    `qa_runs.platform_id`.
...
mod tests {
    ...
    fn queue_am(id: Uuid, tenant: Uuid, run_id: Uuid) -> run_queue::ActiveModel {
        run_queue::ActiveModel {
            id: ActiveValue::Set(id),
            tenant_id: ActiveValue::Set(tenant),
            platform_id: ActiveValue::Set(uuid(0x11)),
