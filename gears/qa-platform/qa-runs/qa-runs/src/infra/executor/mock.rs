    ...
    fn script_for(&self, execution_ref: &ExecutionRef) -> Vec<ExecutionEvent> {
        ...
        let mut events = match self.scripts.get(run_id) {
            ...
            None => self
                ...
                .map_or_else(Vec::new, default_script),
        };
...
impl MockRunExecutor {
    ...
    #[must_use]
    pub fn submitted(&self) -> Vec<RunSpec> {
...
mod tests {
    ...
    use crate::domain::ports::run_executor::{
        EnvSource, ExecutionNode, KubeconfigMount, RunEnv, SecretRef,
    };
    ...
    /// environment holds it. `env_assembly` never models the secret at all
    ...
    async fn the_recorded_spec_carries_the_secret_as_a_reference_and_never_a_value() {
        let executor = MockRunExecutor::new();
        let assembled = env_assembly::assemble(EnvInputs {
            statics: vec![EnvVar {
    ...
    /// The composition [`KubeconfigMount`] can only document: the mount path
    /// and the `KUBECONFIG` variable are produced by two modules and must
    /// agree. The source system writes both from one literal
    /// (`manager/src/services/argo.rs:504-521`); here Task 14 must, and this is
    /// what "must" looks like when it holds.
    #[tokio::test]
    async fn the_kubeconfig_mount_path_matches_the_assembled_kubeconfig_variable() {
        let executor = MockRunExecutor::new();
        let mount_path = "/.kube/kubeconfig".to_owned();
        let assembled = env_assembly::assemble(EnvInputs {
            kubeconfig_path: Some(mount_path.clone()),
            ..EnvInputs::default()
        });
        let mut submission = spec_for(Uuid::new_v4());
        submission.env = RunEnv::new(assembled, BTreeMap::new());
        submission.kubeconfig = Some(KubeconfigMount {
            secret: SecretRef::new("credstore://platform-kubeconfig"),
        ...
        let recorded = executor.submitted();
        let mount = recorded[0].kubeconfig.as_ref().unwrap();
        assert_eq!(
            recorded[0].env.get("KUBECONFIG"),
            Some(&EnvSource::Value(mount.mount_path.clone())),
            "the executor would otherwise resolve the kubeconfig to a path the \
             run's KUBECONFIG does not name"
        );
        assert_eq!(mount.secret.as_str(), "credstore://platform-kubeconfig");
    }
}
