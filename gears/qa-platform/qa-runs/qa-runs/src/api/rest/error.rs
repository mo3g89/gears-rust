...
///   [`as_schedule_error`] documents. No queue path raises one today; the arm
///   is here so that the next one added is not silently a run's.
///
...
impl From<DomainError> for CanonicalError {
    ...
    fn from(e: DomainError) -> Self {
        match &e {
            ...
            // type of its own.
            //
            // Neither names a resource *instance*: the variants carry a
            // platform id and a cluster-wide limit respectively, and neither is
            // a run this gear could name. The platform stays in the violation
            // description, where it is a fact about the refusal rather than a
            // claim about which row the caller should go and look at.
            DomainError::QueueFull {
                platform_id,
                queued,
                limit,
            } => RunResourceError::resource_exhausted(format!(
                "Run queue for platform {platform_id} is full: {queued} of {limit} slots used"
            ))
            // The subject is the *setting*, which is what an operator changes.
            .with_quota_violation(
                "queue_max_depth",
                format!("{queued} of {limit} queued rows for platform {platform_id}"),
            )
            .create(),
...
mod tests {
        ...
        vec![
            ...
            (
                DomainError::InvalidParameters(
                    crate::domain::params::validate(&[qa_runs_sdk::RunParameter {
                        name: String::new(),
                        value: "v".to_owned(),
                    }])
    ...
    fn both_capacity_refusals_are_429_and_name_their_setting() {
        ...
        assert!(
            ...
            "a platform id is not a resource this gear owns, so it must not be \
    ...
    fn invalid_launch_parameters_are_400_and_name_the_parameter() {
        let inner = crate::domain::params::validate(&[qa_runs_sdk::RunParameter {
