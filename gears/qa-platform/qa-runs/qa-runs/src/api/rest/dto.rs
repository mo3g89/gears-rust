...
pub struct RunDto {
    ...
    /// Platform application version snapshotted at launch, not re-derived.
...
/// The 200 body of a force start: the run that now holds the platform.
///
...
pub struct LaunchRunReq {
    pub target: RunTargetDto,
    /// `null` launches a run with no target platform. Such a run is never
...
impl LaunchRunReq {
    ...
    /// the platform's tenancy, whether the plan resolves - needs I/O or the
    ...
    /// `manager/src/services/argo.rs:369-372`): no per-platform queue, no
...
pub struct ScheduleDto {
    ...
    pub name: String,
    pub target: RunTargetDto,
    /// `null` schedules a run with no target platform. Such a run is never
    /// queued and never blocks anything.
    pub platform_id: Option<Uuid>,
    /// Branch each fire resolves against. `null` falls back to the platform's
...
pub struct NewScheduleReq {
    ...
    pub name: String,
    pub target: RunTargetDto,
    /// `null` schedules a run with no target platform.
    pub platform_id: Option<Uuid>,
...
pub struct QueueQuery {
    /// Absent spans every platform in the same window, as the source system's
    /// does.
    pub platform_id: Option<Uuid>,
...
mod tests {
    use super::{
        ...
        QueueQuery, RunDetailDto, RunDto, RunTargetDto, ScheduleDto,
    ...
    fn request() -> LaunchRunReq {
        LaunchRunReq {
            target: plan_target(),
            platform_id: Some(Uuid::from_u128(0xB1)),
            branch: None,
            include_tags: vec![],
            exclude_tags: vec![],
            parameters: vec![],
    ...
    fn the_queue_query_survives_absent_fields_and_ignores_odata_parameters() {
        ...
        assert_eq!(empty.platform_id, None);
        ...
        assert_eq!(mixed.platform_id, None);

        let scoped: QueueQuery =
            serde_urlencoded::from_str("platform_id=00000000-0000-0000-0000-0000000000b1")
                .expect("a platform filter parses");
        assert_eq!(scoped.platform_id, Some(Uuid::from_u128(0xB1)));
    }
    ...
    /// things. `platform_id` is the one that matters most - a replace that drops
    /// it detaches a platform-targeted schedule, and the runs it then fires are
    ...
    fn omitting_a_defaulted_field_clears_it_on_a_replace() {
        ...
        full["platform_id"] = serde_json::json!(Uuid::from_u128(0xB1));
