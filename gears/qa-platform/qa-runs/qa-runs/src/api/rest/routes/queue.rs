...
pub(super) fn register_queue_routes(mut router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    ...
    router = OperationBuilder::get("/qa/v1/queue")
        ...
        .description(
            "One page of run-queue rows, newest first, all states. `queue_position` is \
             ...
             platform_id is the way to get a position you can rely on. `limit` defaults to \
        ...
        .query_param("platform_id", false, "Narrow to one platform's queue")
        .query_param_typed(
            "limit",
            false,
    ...
    router = OperationBuilder::delete("/qa/v1/queue/{id}")
        ...
        .description(
            "Drop a row that has not started, retiring its run in the same transaction. \
             ...
             its platform, and dropping it here would release a platform a live execution \
             still owns.",
        )
        .tag(API_TAG)
        .authenticated()
    ...
    OperationBuilder::post("/qa/v1/queue/{id}/force-start")
        ...
        .description(
            "Start a queued row now, bypassing the platform occupancy check. The \
             ...
             exclusivity on one platform, but not the limit that protects the whole \
             cluster. The override is logged with the run and the platform named.",
        )
        .tag(API_TAG)
        .authenticated()
        .require_license_features::<License>([])
