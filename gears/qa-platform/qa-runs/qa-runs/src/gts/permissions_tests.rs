//! Unit tests for qa-runs' GTS permission catalog.
//!
...
const EXPECTED_PERMISSION_IDS: &[&str] = &[
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_cancel.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_create.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_dispatch.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_force_start.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_get.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.queue_entry_list.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.run_cancel.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.run_create.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.run_dispatch.v1"),
    ...
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.schedule_get.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.schedule_list.v1"),
    gts_id!("cf.toolkit.authz.permission.v1~cf.qa.runs.schedule_update.v1"),
];

fn qa_runs_permission_instances() -> Vec<&'static InventoryInstance> {
...
#[test]
fn all_qa_runs_permissions_are_registered_in_inventory() {
    let entries = qa_runs_permission_instances();
    assert_eq!(
        entries.len(),
...
fn the_inventory_covers_every_expected_id() {
    let found: std::collections::BTreeSet<_> = qa_runs_permission_instances()
        .iter()
        .map(|e| e.instance_id)
        .collect();
    ...
    assert_eq!(
        found.len(),
        EXPECTED_PERMISSION_IDS.len(),
        "inventory contains qa-runs permission ids not in the expected set"
    );
}
...
fn expected_ids_match_the_derivation_rule() {
    let instances = qa_runs_permission_instances();
    for &expected in EXPECTED_PERMISSION_IDS {
        let entry = instances
            .iter()
...
/// silent: a pair enforced with no catalog entry is an action no role can
/// grant, and a catalog entry with no enforcement is a grant that authorizes
/// nothing.
#[test]
fn the_catalog_matches_the_enforced_surface() {
    let cataloged: std::collections::BTreeSet<(String, String)> = qa_runs_permission_instances()
        .iter()
