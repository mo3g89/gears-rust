...
//!
//! # A fourth obligation, added at Task 18: [`validate`] runs **twice**
//!
//! Its reserved set is now the platform floor unioned with the target
//! product's plugin contract, and a plugin is not knowable without reading the
//! environment and resolving it — I/O, which obligation 3's ordering forbids at
//! launch. So:
//!
//! * **Launch** calls it with `RunVarContract::default()`, before any I/O. That
//!   is the floor, and it is what keeps a rejected launch from forcing a read.
//! * **Dispatch** calls it again with the resolved plugin's contract, in
//!   `service::dispatch_spec::build_spec`, where the plugin is already in hand
//!   for `prepare_run_access`. That is the only place the product's own names
//!   exist.
//!
//! Both calls run every check, which is cheap (at most 50 parameters) and
//! idempotent. The alternative — resolving a plugin at launch purely to
//! validate — would make an illegal parameter's error message depend on
//! qa-catalog being reachable, and would replace a clear `TEST_FILES is
//! reserved` with `the product plugin could not be resolved`.
//!
//! For every plugin shipped today the union adds **nothing**: VHP reserves
//! `E2E_K8S_NAMESPACE` and `KUBECONFIG`, and both are already on the floor.
...
///
/// # The platform **floor** since Task 18
///
/// This list is no longer the whole reserved set: a product plugin declares
/// its own names in `RunVarContract::reserved`, and [`validate`] refuses the
/// **union**. The union is computed by `RunVarContract::union_with_floor`,
/// which takes this array — so a plugin can only ever *add*. It cannot shrink
...
pub fn validate(parameters: &[RunParameter], contract: &RunVarContract) -> Result<(), ParamError> {
    ...
    for parameter in parameters {
        ...
        // as well.
        if reserved.contains(&parameter.name.to_ascii_uppercase()) {
        ...
        // coexist — see `domain::runvars::assemble`, which matches names
...
mod tests {
    ...
    /// [`super::validate`] with a plugin that reserves nothing — the platform
    /// floor alone.
    ///
    /// Every test below predates Task 18's union and asserts the floor's own
    /// behaviour, which is unchanged: this is what "a plugin cannot shrink the
    /// floor" looks like from the inside, and it keeps those assertions reading
    /// as they always did instead of carrying a `RunVarContract::default()`
    /// each.
    fn floor(parameters: &[RunParameter]) -> Result<(), ParamError> {
        validate(parameters, &RunVarContract::default())
    }
    ...
    fn a_valid_set_passes() {
        assert!(floor(&[p("FOO", "1"), p("_bar2", "x")]).is_ok());
        assert!(floor(&[]).is_ok());
    }

    #[test]
    fn an_empty_name_is_rejected() {
        let err = floor(&[p("", "1")]).unwrap_err();
    ...
    fn a_name_must_start_with_a_letter_or_underscore() {
        for bad in ["1FOO", "-FOO", " FOO", "9", "."] {
            let err = floor(&[p(bad, "1")]).unwrap_err();
            assert!(
    ...
    fn every_reserved_name_is_rejected_case_insensitively() {
        for reserved in RESERVED_NAMES {
            for spelling in [
                ...
                let err = floor(&[p(&spelling, "1")]).unwrap_err();
                assert!(
    ...
    fn duplicates_collide_case_insensitively() {
        let err = floor(&[p("FOO", "1"), p("foo", "2")]).unwrap_err();
        assert!(
    ...
    fn the_count_cap_is_reported_before_any_size_violation() {
        ...
        let err = floor(&many_huge).unwrap_err();
        assert!(
    ...
    fn size_violations_are_reported_before_any_name_violation() {
        let huge = "x".repeat(MAX_VALUE_LEN + 1);
        let err = floor(&[p("", "1"), p("BIG", &huge)]).unwrap_err();
        assert!(
    ...
    fn a_name_longer_than_128_chars_is_rejected() {
        ...
        assert!(floor(&[p(&ok, "v")]).is_ok());
        let too_long = "A".repeat(MAX_NAME_LEN + 1);
        let err = floor(&[p(&too_long, "v")]).unwrap_err();
    ...
    fn a_value_larger_than_eight_kib_is_rejected() {
        ...
        assert!(floor(&[p("BIG", &ok)]).is_ok());
        let too_big = "x".repeat(MAX_VALUE_LEN + 1);
        let err = floor(&[p("BIG", &too_big)]).unwrap_err();
    ...
    fn value_size_is_measured_in_bytes() {
        ...
        let err = floor(&[p("BIG", &value)]).unwrap_err();
        assert!(
            matches!(err, ParamError::ValueTooLarge { .. }),
    ...
    fn name_size_is_measured_in_bytes() {
        ...
        let err = floor(&[p(&name, "v")]).unwrap_err();
        assert!(
    ...
    fn contract(names: &[&str]) -> RunVarContract {
        RunVarContract {
            reserved: names.iter().map(|name| (*name).to_owned()).collect(),
        }
    }

    /// **A plugin cannot shrink the floor.** A contract reserving nothing still
    ...
    /// argument depends on.
    #[test]
    fn a_plugin_that_reserves_nothing_still_cannot_take_test_files() {
        let empty = contract(&[]);
        for name in RESERVED_NAMES {
            let err = validate(&[p(name, "1")], &empty).unwrap_err();
            assert!(
                matches!(err, ParamError::Reserved { .. }),
                "{name} must stay reserved whatever a plugin declares"
            );
        }
    }

    /// A plugin's own name is refused on top of the floor, case-insensitively
    /// on both sides: `union_with_floor` uppercases what it is given, and the
    /// check uppercases the parameter.
    #[test]
    fn a_plugins_reserved_name_is_refused_in_any_casing() {
        let vhp_like = contract(&["product_endpoint"]);
        for spelling in ["PRODUCT_ENDPOINT", "product_endpoint", "Product_Endpoint"] {
            let err = validate(&[p(spelling, "1")], &vhp_like).unwrap_err();
            assert!(
                matches!(err, ParamError::Reserved { .. }),
                "{spelling} is the plugin's name however it is written"
            );
        }
        assert!(
            validate(&[p("PRODUCT_ENDPOINT_2", "1")], &vhp_like).is_ok(),
            "and the match is exact, not a prefix"
        );
    }

    /// A name **no** side reserves still passes. Without this, the two tests
    /// above would also pass against a `validate` that refused everything.
    #[test]
