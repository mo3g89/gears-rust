...
//! `cpt-cf-qa-principle-executor-port` (DESIGN §2.1, `cpt-cf-qa-principle-executor-port`), each amended
...
//! DESIGN §3.4, the execution-event table ("`watch` → execution event/log stream") and by
//! `cpt-cf-qa-nfr-run-duration`, whose allocation is explicit that
//! `watch(execution_id)` must *resume* after a control-plane restart
//! (DESIGN §1.2, `cpt-cf-qa-nfr-run-duration`). So this one operation is specified, not ported, and its
...
pub struct RunSpec {
    ...
    /// execution backend alone" (PRD §5.4, `cpt-cf-qa-fr-runs-exclusivity`). Both are set, and the
...
pub trait RunExecutor: Send + Sync {
    ...
    /// restart, and DESIGN §1.2, `cpt-cf-qa-nfr-run-duration` allocates that to "`watch(execution_id)`
    /// resumes".
    ///
