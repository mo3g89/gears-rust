...
//!    [`crate::domain::env_assembly`] already produces.
...
//! secret volume (`argo.rs:504-521`). [`crate::domain::env_assembly`]
...
//! (`env_assembly.rs`, composition obligation 4). This port is the other half
...
//!   execution proceeds**.
//! * [`KubeconfigMount`] — the target platform's kubeconfig. The source system
//!   mounts it as a plain secret volume with no `optional` flag
//!   (`argo.rs:506-511`), whose Kubernetes default is required: an unresolvable
//!   reference means the workload never starts.
//!
...
pub enum EnvSource {
    /// A literal value, as assembled by [`crate::domain::env_assembly`].
    Value(String),
...
///
/// `BTreeMap`, matching [`crate::domain::env_assembly::assemble`]'s own choice
...
impl RunEnv {
    ...
    /// [`crate::domain::env_assembly`] documents at length. Either route
...
/// The target platform's kubeconfig: a reference, plus where the execution puts
/// what it resolves.
///
/// # The obligation this places on the dispatcher
///
/// [`Self::mount_path`] **must equal** the `KUBECONFIG` entry
/// [`crate::domain::env_assembly::assemble`] placed in [`RunSpec::env`]. The
/// source system cannot get this wrong because one `if let` writes both the
/// volume mount and the environment variable (`argo.rs:504-521`) — though note
/// it writes them as *two* literals, not one: the volume mounts the whole
/// secret at the directory `/.kube` (`argo.rs:514`) and the variable names the
/// file inside it, `/.kube/kubeconfig` (`argo.rs:519`), with the file name
/// coming from the secret's key. This field is deliberately the **file** path
/// rather than that directory, which turns a prefix relationship the executor
/// would have to reconstruct into an equality the caller can assert.
///
/// Here the two values are produced by two modules on two different tiers — env
/// assembly applies `KUBECONFIG` last of all, after run parameters — so the
/// port can only name the obligation, not enforce it: **build
/// `EnvInputs::kubeconfig_path` and this field from one value.** Task 14 owns
/// that, and
/// `the_kubeconfig_mount_path_matches_the_assembled_kubeconfig_variable`
/// demonstrates the composed result.
///
/// Deriving the environment entry from this field instead was considered and
/// rejected: env assembly's `KUBECONFIG`-wins-outright ordering is itself
/// ported behaviour with its own test, and a second writer would put that
/// ordering in two places.
#[domain_model]
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct KubeconfigMount {
    /// credstore reference to the platform's kubeconfig
    /// (`qa_environments_sdk::Platform::kubeconfig_credstore_ref`). Required:
    /// unlike [`EnvSource::Secret`], a kubeconfig that does not resolve must
    /// fail the execution rather than silently target nothing — the source
    /// system mounts it as a secret volume with no `optional` flag
    /// (`argo.rs:506-511`).
    pub secret: SecretRef,
    /// Filesystem path the resolved kubeconfig **file** is placed at, inside
    /// the execution. `/.kube/kubeconfig` in the source system
    /// (`argo.rs:517-520`), where the enclosing directory is what the secret
    /// volume mounts (`argo.rs:512-516`).
...
pub struct ExecutionNode {
    ...
    /// (`:461-469`) — the same asymmetry [`crate::domain::env_assembly`]
...
pub struct RunSpec {
    ...
    pub env: RunEnv,
    /// The target platform's kubeconfig. `None` for a run with no platform,
    /// exactly as the source system mounts nothing in that case
    /// (`argo.rs:504`).
    pub kubeconfig: Option<KubeconfigMount>,
