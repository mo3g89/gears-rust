...
fn every_resource_descriptor_carries_its_own_name_const() {
    assert_eq!(
        resources::RUN.name(),
        resources::RUN_NAME,
        "resources::RUN was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::QUEUE_ENTRY.name(),
        resources::QUEUE_ENTRY_NAME,
        "resources::QUEUE_ENTRY was built from the wrong *_NAME const",
    );
    assert_eq!(
        resources::SCHEDULE.name(),
        resources::SCHEDULE_NAME,
        "resources::SCHEDULE was built from the wrong *_NAME const",
    );
}
