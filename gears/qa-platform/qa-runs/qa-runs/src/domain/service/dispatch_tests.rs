...
/// The kubeconfig composition [`KubeconfigMount`] can only document: the mount
/// path and the `KUBECONFIG` variable are produced on two tiers and must agree.
/// Here they come from one constant, and this is what "must" looks like when it
/// holds in production code rather than in a hand-built spec.
#[tokio::test]
async fn the_kubeconfig_mount_and_the_assembled_variable_come_from_one_value() {
    ...
    let mount = spec.kubeconfig.as_ref().expect("a platform run mounts one");
    assert_eq!(
    ...
    assert_eq!(mount.secret.as_str(), "credstore://kubeconfig");
}
...
async fn a_platformless_run_carries_no_kubeconfig() {
    ...
    let spec = &executor.submitted()[0];
    assert!(spec.kubeconfig.is_none());
    assert!(!fakes::env_values(spec).contains_key("KUBECONFIG"));
}

/// `env_assembly`'s composition obligation 2: `APP_VERSION` and `APP_BUILD` come
