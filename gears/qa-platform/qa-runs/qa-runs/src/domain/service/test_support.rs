...
///
/// `get_platform` answers only for a platform whose owning tenant matches the
...
pub(super) struct MockEnvironments {
    platforms: Vec<(Uuid, TargetPlatform)>,
    lookups: Mutex<Vec<Uuid>>,
}

impl MockEnvironments {
    pub(super) fn with_platform(tenant_id: Uuid, platform: TargetPlatform) -> Self {
        Self {
...
pub(super) fn platform_fixture(version: Option<&str>, build: Option<&str>) -> TargetPlatform {
    let now = OffsetDateTime::now_utc();
    TargetPlatform {
        id: PLATFORM_ID,
        ...
        product_id: None,
        description: None,
        kubeconfig_credstore_ref: "credstore://kubeconfig".to_owned(),
        available: true,
        ...
        // Task 7); this fixture predates it and has nothing to say about
        // observation, so every one is `None`.
        vhp_base_url: None,
...
pub(super) fn platform_fixture_with_branch(default_branch: Option<&str>) -> TargetPlatform {
    TargetPlatform {
...
impl QaEnvironmentsClientV1 for MockEnvironments {
    ...
    async fn list_platforms(
        ...
        Err(environments_unsupported("list_platforms"))
    }
    ...
    async fn create_platform(
        ...
        Err(environments_unsupported("create_platform"))
    }
    ...
    async fn update_platform(
        ...
        Err(environments_unsupported("update_platform"))
    }
    ...
    async fn delete_platform(
        ...
        Err(environments_unsupported("delete_platform"))
    }
