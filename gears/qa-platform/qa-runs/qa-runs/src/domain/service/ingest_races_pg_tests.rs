...
impl qa_environments_sdk::QaEnvironmentsClientV1 for UnreachableEnvironments {
    async fn get_platform(
        ...
        _id: Uuid,
    ) -> Result<qa_environments_sdk::TargetPlatform, qa_environments_sdk::QaEnvironmentsError> {
        unreachable!("a platformless run must not reach the environments client")
    }
    async fn list_platforms(
        &self,
        _ctx: &SecurityContext,
    ) -> Result<Vec<qa_environments_sdk::TargetPlatform>, qa_environments_sdk::QaEnvironmentsError>
    {
        unreachable!("a platformless run must not reach the environments client")
    }
