...
use crate::domain::ports::run_executor::{ExecutionNode, RunAccess, RunEnv, RunSpec, RunnerSpec};
use crate::domain::repos::{QueueRepository, RunsRepository};
use crate::domain::runvars::{self, RunVar, RunVarInputs};

/// What a run's product plugin says about reaching its target environment.
///
/// Three calls, one struct, because they are made together from one resolved
/// plugin and consumed together by one `RunSpec` — and because the alternative
/// is a three-tuple whose second and third elements are both plugin-declared
/// shapes a caller could transpose.
struct PluginDispatch {
    /// Mounts and a service account, straight onto the spec. Its
...
fn plugin_error(detail: &'static str) -> DomainError {
    DomainError::Internal(detail.to_owned())
}

// `AMBIGUOUS_CREDENTIAL` was deleted by Task 19.
//
...
// pre-plugin `kubeconfig` field for a plugin with no sole required secret,
// with `LEGACY_CREDENTIAL_UNBINDABLE`. Refusing at the write is strictly
// better -- the operator is told when they submit rather than at the next
// dispatch.

/// A plugin that refused without saying why. Every shipped plugin classifies
/// its failures, so this is the shape of a plugin that does not.
const ACCESS_REFUSED: &str =
    "this run's product plugin refused to prepare access to the target environment";
...
/// the skipped ones, which is why `runvars` refuses to blank-filter the list
/// as a whole (its composition obligation 3).
fn push_if_present(into: &mut Vec<RunVar>, name: &str, value: Option<&str>) {
    ...
    Q: QueueRepository,
{
    ...
    async fn plugin_dispatch(
        ...
        ctx: &SecurityContext,
        environment_id: Uuid,
    ) -> Result<PluginDispatch, DomainError> {
        let environment = self
            .environments
            .get_environment(ctx, environment_id)
            .await
        ...
        // observed" from "observed and found nothing", and dispatch reaches
        // environments in the first state — a run may be launched against an
        // environment the observation cycle has not reached yet. Same
        // expression as `qa-environments`' observation path, deliberately.
        let observed =
        ...
        let access = plugin
            .prepare_run_access(&handle)
            .await
            .map_err(|failure| {
                // `failure.detail` only. It is a fixed `&'static str` by
        ...
        Ok(PluginDispatch {
            access,
            runner: plugin.runner(observed),
            contract: plugin.env_contract(),
        })
    }
    ...
    ///
    /// `runvars`'s composition obligation 1 says assembly happens at
    ...
    /// set. `E2E_K8S_NAMESPACE` is no longer on this list: it has a source, and
    ...
    async fn build_spec(
        ...
        // A run with no target environment gets the empty shapes, which is
        // what the source system does in that case: it mounts nothing
        // (`argo.rs:504`) and issues no platform query at all (`:489-492`).
        let (mut access, runner, contract) = dispatch.map_or_else(
            || {
                (
                    RunAccess::default(),
                    RunnerSpec::default(),
            ...
            |dispatch| (dispatch.access, dispatch.runner, dispatch.contract),
        );
        ...
        let plugin_env: Vec<RunVar> = std::mem::take(&mut access.env)
            .into_iter()
            .map(RunVar::from)
            .collect();

        // **The union, and the only place it can be checked.** `params::validate`
        // ran at launch against the platform floor alone; the plugin's own
        // reserved names are not knowable until a plugin is resolved, which
        // needs the environment, which needs I/O. So the floor is enforced
        // before any read (launch step 1) and the product's addition here.
        //
        // For every plugin shipped today the union adds nothing: VHP reserves
        // `E2E_K8S_NAMESPACE` and `KUBECONFIG`, both already on the floor. The
        // check exists for the product that reserves a name of its own, and
        // `validate` unions with the floor itself so a contract cannot shrink
        // it.
        params::validate(&run.parameters, &contract)?;
        ...
        // `runvars::assemble_collect_vars` owns the frozen names.
        if let RunTarget::Collect { collect_url, .. } = &run.target {
            statics.extend(
                runvars::assemble_collect_vars(collect_url)
                    .into_iter()
                    .map(|(name, value)| RunVar { name, value }),
            );
        }
        ...
        let env = runvars::assemble(RunVarInputs {
            ...
            parameters: run.parameters.iter().cloned().map(RunVar::from).collect(),
        });
