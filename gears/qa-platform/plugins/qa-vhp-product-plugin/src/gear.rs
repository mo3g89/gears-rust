...
//!
//! Everything else in this crate is a library: schemas, detection rules,
//! [`observe`](crate::observe) and [`run`](crate::run) are only functions
//! until something registers them. This module is that something — it
//! publishes a `PluginV1<QaProductPluginSpecV1>` instance to the GTS
//! types-registry and registers the plugin object in the `ClientHub` under
//! that instance id, which is the pair Task 12's `QaProductRegistry` resolves
//! a product's plugin through.
//!
...
///
/// Nothing about *what* this plugin does is configurable: the VHP detection
/// rules are frozen (see [`crate::detect`]) and the run variables are the
/// product's, not a deployment's. Both fields exist so that a deployment
/// running more than one product plugin can say which one a product resolves
/// to, and in what order candidates are considered.
#[derive(Debug, Clone, Deserialize)]
#[serde(default, deny_unknown_fields)]
pub struct VhpProductPluginConfig {
...
impl Default for VhpProductPluginConfig {
    fn default() -> Self {
        Self {
...
#[async_trait]
impl Gear for VhpProductPluginGear {
    ...
    async fn init(&self, ctx: &GearCtx) -> anyhow::Result<()> {
        let cfg: VhpProductPluginConfig = ctx.config_or_default()?;
        info!(
            vendor = %cfg.vendor,
        ...
        // terminal.
        let plugin: Arc<dyn QaProductPluginV1> = Arc::new(VhpProductPlugin::new());
        let registered = RegisteredPlugin::new(plugin)
            .map_err(anyhow::Error::new)
            .context("qa-vhp-product-plugin: invalid schemas")?;
        ...
        info!(instance_id = %instance_id, "VHP product plugin registered");
        Ok(())
    }
}
